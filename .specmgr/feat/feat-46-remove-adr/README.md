...
status: planning
type: feat
updated: '2026-09-04 21:09:30.436+02:00'
version: 1.0.0
---
...
- REQ-002: Every reference to a converted ADR id anywhere in the repository (source code, `docs/`, `.specmgr/`, `README.md`, `AGENTS.md`, including historical session transcripts) is updated from `ADR <uuid>` to `DEC <uuid>` for that same uuid.
- REQ-003: The `adr` MCP domain package (`src/biz/dfch/specmgr/adr/` — tools, resources, prompts, data) and its schema package (`src/biz/dfch/specmgr/models/adr/`) are removed from `src/`.
...
- [ ] ACC-003: `src/biz/dfch/specmgr/adr/` and `src/biz/dfch/specmgr/models/adr/` no longer exist on disk.
...
- [ ] ACC-005: `general/tools/set_status.py`, `_path_safety.py`, `general/resources/config.py` contain no remaining reference to `"adr"` as a supported type/domain.
...
- [ ] ACC-008: `README.md`'s artifact-type list no longer mentions ADR as a current or deprecated type, and `AGENTS.md` no longer describes an `adr` domain bullet or a `models/adr/` exception paragraph.
...
- A repo-wide find/replace of `ADR <uuid>` to `DEC <uuid>` (same uuid) for all 30 converted ids, across `src/`, `docs/`, `.specmgr/` (including historical session transcripts), `README.md`, and `AGENTS.md`.
- Removing the `adr/` and `models/adr/` source packages and their tests.
- Removing ADR special-casing from `general/tools/` (`set_status`, `_path_safety`, `config` resource), `server.py`, `cli.py`, and `commands/` (`adr_toc`, `docs`, `schema`, `mcp_docs`).
- Removing the `specmgr-adr-toc` pre-commit hook and its CI step.
- Updating `README.md`/`AGENTS.md` prose that describes the ADR domain as a current or deprecated artifact type.
...
- [ ] Task 1.4: Spot-check a sample of converted DEC documents against their original ADR content for fidelity (no lost decision text/options).

#### Phase 2: Update All References Repo-Wide

- [ ] Task 2.1: For each of the 30 preserved uuids, find every `ADR <uuid>` occurrence across `src/`, `docs/`, `.specmgr/`, `README.md`, `AGENTS.md`.
- [ ] Task 2.2: Replace each `ADR <uuid>` citation with `DEC <uuid>` (same uuid), preserving surrounding text/formatting.
- [ ] Task 2.3: Update prose descriptions of the artifact type itself (not just id citations) in `README.md`'s artifact list and `AGENTS.md`'s ADR bullet / models-location paragraph / tool-listing sections.
...
- [ ] Task 4.4: Update docstrings/comments in `general/models/summary.py` and `general/tools/__init__.py` that reference adr.
...
- [ ] Task 4.6: Re-run vulture/`whitelist.py` check for any new dead-code false positives left behind by the removal.
...
- [ ] Task 6.2: Manually review `README.md` and `AGENTS.md` end-to-end for any remaining ADR-as-artifact-type language.
