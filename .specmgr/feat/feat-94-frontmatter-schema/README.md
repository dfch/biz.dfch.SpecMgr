...
> **Superseded timestamp-format decisions (feat-146-date-time, 2026-09-24):** ADR `8c889262-152b-4b8e-ae2c-75371f7a9edf` supersedes the space-only frontmatter pattern this feature pinned
> into every `*_schema.json` (and the `T`-separator migration it explicitly dropped): `created`/`updated` now accept `T` or space, the MCP write side emits `T`-separated values, and the
> schemas regenerated after feat-146 carry the widened `[T ]` pattern.
