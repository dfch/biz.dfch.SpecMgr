...
status: planning
type: feat
updated: '2026-09-19 14:02:49.785+02:00'
version: 1.0.0
---
...
- **VCR maintenance**: each of Phases 2 through 6 below ends with a task extending the relevant initial VCR(s) (created in Phase 0) with concrete test references for whatever acceptance criteria that phase just made verifiable, per `.specmgr/conventions.md`'s VCR-early convention; Phase 9 confirms every VCR has reached `## Coverage: full`. Phase 6 now extends **both** the logging VCR (log-record redaction) and the telemetry VCR (span redaction, a criterion its initial Phase-0 draft did not yet carry an AC for) given the widened redaction scope above.
- **Risk flagged in the ADR**: `Server.middleware` is documented as a provisional API in the installed SDK ("signature and semantics change ... before v2 final") -- Phase 4 adds a dedicated compatibility test asserting the fail-open behavior above, so a breaking SDK change is caught loudly (via the test) while the running server itself degrades quietly (via the fail-open policy). No upper version bound is added to the `mcp` dependency to further mitigate this (see Explicitly Out Of Scope) -- the compatibility test and the runtime fail-open policy are considered sufficient.
...
- [ ] Task 1.1: Add `telemetry/config.py` parsing the eight env vars above (including the two file-sink vars) into a typed, validated config object, all defaulting to disabled; raise (e.g. `ValueError`) immediately on an invalid/incomplete combination (`SPECMGR_LOG_FILE_ENABLED=true` without a non-blank `SPECMGR_LOG_FILE_PATH`, or an `SPECMGR_LOG_FORMAT`/`SPECMGR_OTEL_EXPORTER` value outside its closed set) -- static misconfiguration fails closed, per the Design Notes above (ACC-011).
- [ ] Task 1.2: Unit tests for config parsing (defaults, valid values, and each invalid/incomplete combination raising).
...
- [ ] Task 5.2: Tool-call latency histogram + call counters by tool/domain, in the middleware, via an OTel `Meter` (no-op when disabled), named per the MCP-specific scheme (`mcp.tool.duration`, `mcp.tool.name`, `mcp.domain`), using the Task 5.1 mapping.
...
- [ ] Task 5.5: Add wait-time timing to each of the 13 `<domain>/tools/_lock.py` files' lock context manager, recorded to the same meter.
- [ ] Task 5.6: Implement graceful OTLP-unreachable degradation: at most one stderr message per failure episode, a further single message only after a subsequent reconnect-then-drop cycle, never spamming. Per the Design Notes' flagged risk, investigate the OTel SDK's own exporter/processor failure-logging behavior first (it likely logs per failed export cycle, not once) and implement explicit de-duplication (e.g. a `logging.Filter` with episode-tracking state on the exporter's own logger name) rather than assuming the SDK already provides this.
- [ ] Task 5.7: Unit tests for each counter/histogram, for the Task 5.1 domain mapping, and for the OTLP-unreachable degradation/de-duplication behavior.
- [ ] Task 5.8: Extend VCR `54fd355f-0978-4dd0-9e1d-da89432e5321` (telemetry) with concrete test references for AC-003/AC-005/AC-006; keep `## Coverage: partial` pending Phase 6's span-redaction extension (AC still outstanding for that criterion as of this phase).
- [ ] Task 5.9: Quality gate.

#### Phase 6: Redaction safeguards

- [ ] Task 6.1: Add `telemetry/redact.py`: a logging `Filter` (scoped to specmgr's own log records) stripping document body content, absolute filesystem paths, and document/artifact titles, plus a **global, `TracerProvider`-level `SpanProcessor`** (`on_end` hook or equivalent) that scrubs the same fields from every span's attributes and exception events -- including spans produced by the SDK's own built-in `OpenTelemetryMiddleware`, not only attributes this feature's own middleware/meter sets (ACC-012; see the widened Design Notes decision above).
- [ ] Task 6.2: Wire the filter into the logging setup from Phase 2, and the span processor into the `TracerProvider` from Phase 4 (so it applies process-wide, not just to spans this feature's own code creates).
- [ ] Task 6.3: Unit tests asserting redaction on representative log payloads (including titles), and asserting the span processor scrubs a span's attributes and exception event even when that span is created entirely by the SDK's built-in `OpenTelemetryMiddleware` (e.g. inject an exception whose message contains a fake title/path and assert the exported span no longer contains it) -- not only spans this feature's own middleware would create.
...
- [ ] Task 6.5: Extend VCR `54fd355f-0978-4dd0-9e1d-da89432e5321` (telemetry) with a new, concrete acceptance criterion and test reference for span-level redaction (a REQ `dacd01f4-ffd8-4363-a20b-5ac1ce11eef2` clause its initial Phase-0 draft did not yet carry an AC for); move `## Coverage` to `full` if every telemetry AC is now demonstrably covered.
