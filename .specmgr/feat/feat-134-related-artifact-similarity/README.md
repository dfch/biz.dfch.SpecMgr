...
status: planning
type: feat
updated: '2026-09-19 12:52:35.766+02:00'
version: 1.0.0
---
...
**As of 2026-09-19**: **Phase 1 complete** (Tasks 1.1-1.6): the design ADR (accepted), the `similarity` extra (`fastembed`, locked), the `EmbeddingProvider` protocol + `FastEmbedProvider` + lazy `get_default_provider()` (`general/tools/_embedding.py`), the presence-based `SPECMGR_SIMILARITY_DISABLED` opt-out + `_similarity_availability()` helper with the structured `SimilarityUnavailableResult` (`general/models/similarity_unavailable.py`), the global content-hash-validated embedding cache (`general/tools/_embedding_cache.py`), and the shared `WHOLE_BODY_DOMAINS` registry (`general/tools/_domains.py`) with all five existing generic tools re-pointed at it -- full suite green (3307 passed), MCP schema verified byte-identical (`specmgr mcp-docs` no-op), generated docs regenerated. **Phase 2 (Text Extraction + Chunking + Ranking, Tasks 2.1-2.4) is next.** Note: the feat frontmatter `status` stays `planning` -- the feat model's own status vocabulary is `planning`/`progress`/`review`/`done` (no `in-progress`; the AGENTS.md vocabulary fix is Task 4.5's scope).
