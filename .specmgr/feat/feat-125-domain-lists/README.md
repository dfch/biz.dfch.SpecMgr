...
**As of 2026-09-22**: Implementation complete -- all five phases done and gate-green; awaiting review. Phase 1 (quick wins: the `sysrs` test gap, the two dead-constant removals, the `feat` docstring rewording), Phase 2 (the shared `general/tools/_domains.py` source, with `_path_safety.py`'s `_UUID_TYPES`/`_TYPE_FEAT` and `set_status.py`'s `_TYPE_ADR` rewired onto it and `validate_id`'s unknown-type error message derived from it), Phase 3 (src rewiring: the five generic tools' `type` signatures PEP 692 re-derived from the shared source, the `_ADAPTERS`/`_ALLOWED_STATUSES_BY_TYPE` key sets normalized and guarded by set-equality asserts, the eight derived description/error-message strings, and `config.py`'s membership assert plus derived description), Phase 4 (test rewiring: all six test-side hand-listed domain constants deleted in favor of the shared source, the two registration tests' expected enums now derived from `list(WHOLE_BODY_DOMAINS)`), and Phase 5 (docs/ADR/conventions: `docs/MCP.md` + `docs/api/` + `docs/GENERATED.md` regenerated with zero residual drift, the new accepted ADR c4efbde6-fd19-4aa8-8668-95316ed62dcc ("Single source of truth for the document-type domain-name set") plus `specmgr adr-toc`, the `.specmgr/conventions.md` "Domain-List Constants" rule, the `AGENTS.md` future-domain convention update, and the `CHANGELOG.md` `[Unreleased]` entry) are done. The issue's 23-site inventory was verified against the code during planning: all sites confirmed (line numbers in the issue are approximate -- e.g. the registration-test inline lists sit at `test_delete.py:791`/`test_update.py:1351` today), plus two extra singletons found (`set_status.py`'s `_TYPE_ADR`, `_path_safety.py`'s `_TYPE_FEAT`), plus one site audited out of scope (`commands/schema.py`'s `_GENERATORS` registry). Both planning-time decisions were resolved with the issue author: the 13-domain set is adr-first (accepting `set_status`'s harmless enum reorder), and the six decorator description strings are derived from the shared source. A 2026-09-21 pre-implementation audit (full tree + issue #125 text) verified the plan against the current code and folded in four deltas -- see Updates/Decisions Made.

### Updates

<!-- Newest entry first -- prepend new entries directly below this comment. -->

#### 2026-09-22 05:20:28.443Z - Phase 5 complete

Phase 5 (docs, ADR, conventions, quality gate) is implemented and gate-green. `specmgr mcp-docs`, `specmgr docs` (466 module pages + `docs/GENERATED.md`), and `specmgr schema` (all 12 schemas "unchanged", exit 0) each regenerated with an empty `git diff` (Task 5.1, REQ-010/ACC-009 precondition). The new ADR c4efbde6-fd19-4aa8-8668-95316ed62dcc ("Single source of truth for the document-type domain-name set") was written `accepted` (2026-09-22), validated with the repo's own `parse_adr`/`render_adr` (parses clean, round-trips byte-identical), and registered in `docs/adr/README.md` via `specmgr adr-toc` (diff = new row only) (Task 5.2). `.specmgr/conventions.md` gains the sibling `### Domain-List Constants` rule after `### Docstring Style` (Task 5.3): the domain names live in ONE place (`general/tools/_domains.py`), every `src/`/`tests/` module imports the shared names instead of hand-listing a set, `WHOLE_BODY_DOMAINS` is the only hand-listed domain tuple in the repo, and docstring prose is the sanctioned exception. `AGENTS.md`'s future-domain convention paragraph now names the `_domains.py` registration step first and cites the new ADR as refining ADR 36905d5b (the other `36905d5b` mentions are historical per-domain bullets and stay untouched), and `CHANGELOG.md`'s `[Unreleased]` gains the Added/Changed/Fixed entry (Task 5.4). The fresh ACC-001 sweep (quoted-domain-name lines >=4 plus a 400-char multi-line window scan over `src/`/`tests/`, plus grep for every known constant name) confirms: the only hand-listed domain tuple left in `src/` is `WHOLE_BODY_DOMAINS` itself; all named constants are gone; every `tests/` site is rewired or matches the sweep ruling (per-domain `_Case`/`_InjectionCase` fixture rows, `test_mcp_docs.py`'s 7-value render fixture, single-name comparisons) -- one pre-existing pattern beyond the ruling's named categories, the per-domain model `test_frontmatter.py` negative-case samples (`for other in ("req", "gol", ...)`; 4/5/7 entries per file, non-exhaustive, last touched by feat-31, zero behavior to keep in sync), is classified as kept and flagged in the Phase 5 report (Task 5.5). Full gate: `ruff format --check` (1696 files already formatted), `ruff check` (All checks passed), `vulture` (clean), `pylint` advisory (8.91/10, stable; all 28 findings on this feature's touched files are pre-existing, none on a feature-changed line), `pytest -n auto --cov=src --cov-report=` (3365 passed), and the four drift checks (`specmgr docs`/`adr-toc`/`mcp-docs`/`schema`) are all no-ops on the second pass -- `git status` shows only the intended Phase 5 files (Task 5.5, ACC-009/011).
