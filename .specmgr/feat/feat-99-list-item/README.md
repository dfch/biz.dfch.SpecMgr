...
**Update 2026-09-09 (Phase 2 complete)**: implemented the reject-not-join fix decided above. `MarkdownListItem.single_line_text(self, *, expected: str) -> str` (`models/md/markdown_list_item.py`) asserts `"\n" not in self.text` and raises an `AssertionError` naming `self._path`/`self._line`, stating plainly that soft-wrapped/lazy-continuation list items are not supported, and telling the author to join the text onto one physical line -- wired into all five confirmed call sites: `tsk.TaskItem.checked`/`.description`, `feat.RequirementItem.description`, `rsk.ThresholdItem.low`/`.high`/`.zone` (all three properties, one call each, matching the existing per-property pattern-duplication style), and `rsk.StrategyItem.strategy` (its module docstring updated to note the new guard). `feat.AcceptanceCriterionItem.criterion_description` needed no direct call -- it reads `self.description` (`TaskItem.description`), which already goes through the guard, so it is covered transitively, exactly as this plan's own Design Notes anticipated. Free-form/`re.DOTALL` usages (`req` Tags, `rsk.QuadrantItem`/`MitigationItem`/`StatusItem`, `sysrs` cross-ref lists, etc.) were left untouched. Full quality-gate-adjacent verification: `ruff format --check`/`ruff check` clean; full `pytest -n auto` suite (3344 tests) passes. Task 2.3 confirmed with no code change needed: built a `feat` body fixture with a soft-wrapped `REQ-001:` bullet and called both the real `validate(type="feat", ...)` and `create_feat(content=...)` Python entry points directly -- both raise/return the new guard's actionable message (`"... expected 'REQ-NNN: <description>', but this item's text spans more than one physical line; soft-wrapped/lazy-continuation list items are not supported here -- join the text onto one physical line."`), wrapped end-to-end with the domain/tool/channel label by the existing `wrap_tool_errors`, confirming ACC-003/REQ-003 remain satisfied with the new guard in place. No scratch verification files were left in the repo. Phase 3 (AGENTS.md/conventions.md documentation, dedicated new unittest coverage, and the full quality gate incl. vulture/docs regen) remains outstanding.

### Updates

<!-- Newest entry first -- prepend new entries directly below this comment. -->

#### 2026-09-09 06:00:00.000Z - Phase 2 implemented: shared MarkdownListItem.single_line_text guard, wired into all five call sites

Implemented the Phase 2 fix: added `MarkdownListItem.single_line_text(self, *, expected: str) -> str` (`models/md/markdown_list_item.py`), a shared guard that asserts `.text` contains no embedded `\n` and raises an `AssertionError` naming `self._path`/`self._line`, stating soft-wrapped/lazy-continuation list items are not supported, and telling the author to join the text onto one physical line. Wired it into `tsk.TaskItem.checked`/`.description` (`tsk/models/v1/task_item.py`), `feat.RequirementItem.description` (`feat/models/v1/body.py`), `rsk.ThresholdItem.low`/`.high`/`.zone` (`rsk/models/v1/risk_matrix.py`, one call per property, matching the existing per-property duplication style), and `rsk.StrategyItem.strategy` (`rsk/models/v1/tara.py`, also updating that module's docstring to note the new guard). `feat.AcceptanceCriterionItem.criterion_description` needed no direct wiring -- it reads `self.description`, i.e. `TaskItem.description`, which already runs the guard, so it is covered transitively; left as-is per the plan's own hint. Left every free-form/`re.DOTALL` usage (`req`'s `## Tags`, `rsk.QuadrantItem`/`MitigationItem`/`StatusItem`, `sysrs` cross-reference lists, `dec.Option`/`sop.Step`/vcr's heading items) untouched. Verified: `ruff format --check` and `ruff check` clean; full `pytest -n auto` suite (3344 tests) passes, including the targeted `tests/models/md/test_markdown_list_item.py`, `tests/tsk/models/v1/test_task_item.py`, `tests/feat/models/v1/test_body.py`, `tests/models/test_risk_matrix.py`, `tests/models/test_tara.py`, `tests/rsk/models/v1/`, and `tests/req/` (free-form list regression) subsets. Confirmed Task 2.3/ACC-003/REQ-003 with a scratch script (not committed) that built a `feat` body fixture with a soft-wrapped `REQ-001:` bullet and called the real `validate(type="feat", full=False, ...)` and `create_feat(content=...)` Python entry points directly: both surfaced the new guard's actionable message end-to-end, prefixed by `wrap_tool_errors`'s domain/tool/channel label (`"feat validate (body): ..."` / `"feat create_feat (body): ..."`), confirming no code change was needed in the MCP error-wrapping layer. No scratch/throwaway files were left on disk.
