...
def _to_summary(doc: UcDocument, path: Path) -> UcSummary:
    result = UcSummary(
...
def _to_failed_summary(path: Path, error: Exception) -> UcSummary:
    result = default_failed_summary(UcSummary, path, error)
    return result
...
@mcp.tool(
    ...
    description=(
        "Ids, titles, statuses, and refs of use cases in the configured use-case base "
        ...
        "for documents that have no assigned id; use it with the get_uc tool instead. "
        "max_results/offset control paging (default page size 25, capped at 100); "
        "out-of-range values are clamped, not errored."
    ),
)
def list_uc(max_results: int | None = None, offset: int | None = None) -> PagedResult[UcSummary]:
    ...
    :func:`~biz.dfch.specmgr.uc.models.v2.parse_uc` raises) appears inline
