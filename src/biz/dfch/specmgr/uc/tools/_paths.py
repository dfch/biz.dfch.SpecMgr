...
from ._cache import read_uc, reconcile_uc_cache
...
#: (``{docs root}/uc/``, e.g. ``docs/uc``).
UC_TYPE_NAME = "uc"


class UcNotFoundError(LookupError):
...
def uc_base_dir() -> Path:
    ...
    """
    result = doc_base_dir(UC_TYPE_NAME)
    return result


def ensure_uc_base_dir() -> Path:
    ...
    """
    result = ensure_doc_base_dir(UC_TYPE_NAME)
    return result


def iter_uc_paths() -> Iterator[Path]:
    ...
    """
    result = iter_doc_paths(uc_base_dir())
    return result


def _get_uc_id(doc: UcDocument) -> str | None:
    """Extract the id from a parsed :class:`UcDocument` (``find_doc_path_by_id``'s ``get_id_fn``)."""
    result = doc.frontmatter.id
    return result


def find_uc_path(base_dir: Path, id_: str) -> Path:
    ...
    try:
        result = find_doc_path_by_id(base_dir, id_, read_uc, _get_uc_id, reconcile_fn=reconcile_uc_cache)
