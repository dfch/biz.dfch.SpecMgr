...
before ever calling ``create_uc``/``update_uc`` (or independently of either),
and is exactly the same check both of those tools already run internally on
...
from ..models.v2 import UseCase, parse_uc


@mcp.tool(
    name="validate_uc",
    title="Validate use case content",
    description=(
        "Disk-free, id-free dry run validating use case content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_uc(content: str, full: bool = False) -> bool:
    ...
    exactly as ``create_uc``/``update_uc`` themselves do.
    ...
    full:
        ...
        shape ``create_uc``/``update_uc`` accept) -- raises ``ValueError``
