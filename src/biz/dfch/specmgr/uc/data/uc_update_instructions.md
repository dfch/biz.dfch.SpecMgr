You are revising an existing Use Case (UC) document, id: $id
...
Call `get_uc(id)` to load the document's current frontmatter and body.
...
- A change to the body -- `Characteristic Information` (`Goal in
  Context`, `Scope`, `Level`, `Preconditions`, `Success End Condition`,
  `Failed End Condition`, `Primary Actor`, `Secondary Actors`,
  `Trigger`, `Frequency`, `Priority`, `Performance Target`, `Channels
  to Primary Actor`, `Channels to Secondary Actors`, `Related Use
  Cases`), `Main Success Scenario`, `Extensions`, `Sub-Variations`,
  `Open Issues`, or `Related Information` (`Notes`, `Assumptions`) --
  -> the generic `update` tool called with `type="uc"`: a **line-range
  replace** for a localized change, or a **whole-body replace**
  otherwise. `content` is body markdown only (no frontmatter block) in
  both cases.
  - **Line-range replace** (a localized change -- one paragraph, field,
    or section): first call `get_uc(id, raw=True)` to see the exact
    ...
    -- and call `update(id, type="uc", content, offset=..., limit=...)`
    ...
    byte-identical. Remember that `Extension`/`Sub-Variation` headings
    (`### Extension {step}{letter}. {condition}` / `### Step {N}:
    {description}`) must keep referencing a real `Main Success
    Scenario` step number, with no duplicate references, or validation
    will reject the write.
  - **Whole-body replace** (a multi-section change, or whenever you are
    uncertain about the line range): call `update(id, type="uc", content)`
...
- A change to `classification` ->
  `set_classification(id, type="uc", classification)` instead --
...
Fetch `specmgr://uc/schema` to confirm field names and constraints
before drafting the replacement body. Optionally call
`validate(type="uc", content=content, full=False)` beforehand to dry-run the new body
