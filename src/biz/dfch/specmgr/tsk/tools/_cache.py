...
__all__ = ["invalidate_tsk_cache", "read_tsk", "reconcile_tsk_cache", "reset_tsk_cache"]
...
def _parse(text: str) -> TskDocument:
    """Parse ``text`` into a :class:`TskDocument` (the cache's own ``parse_fn`` -- Phase 6: text in, not ``Path``).
...
def invalidate_tsk_cache(path: Path) -> None:
...
def reconcile_tsk_cache(live_paths: Iterable[Path]) -> None:
