...
``update_tsk`` and any future task list mutation tool (``set_status_tsk``)
wrap their whole sequence in ``with tsk_lock(id):``.
...
#: One lock per task list id, created lazily on first use and never
...
@contextmanager
def tsk_lock(id_: str) -> Iterator[None]:
