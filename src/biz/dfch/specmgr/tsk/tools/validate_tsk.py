...
"""``@mcp.tool()`` wrapper: validate_tsk (Task 3.7).

Unlike ``validate_adr`` (which is id-based and re-reads a file from disk),
``validate_tsk`` is a **disk-free, id-free dry run**: it validates a
submitted ``content`` string directly, without ever touching the task list
base directory or resolving an id. This lets a caller check a draft before
ever calling ``create_tsk``/``update_tsk`` (or independently of either), and
is exactly the same check both of those tools already run internally on
their own ``content`` argument, exposed standalone here. Mirrors
``req.tools.validate_req`` exactly.
"""
...
@mcp.tool(
    name="validate_tsk",
    title="Validate task list content",
    description=(
        "Disk-free, id-free dry run validating task list content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_tsk(content: str, full: bool = False) -> bool:
    """Validate ``content`` as task list markdown, without reading or writing any file.

    "Validate" means letting :class:`~biz.dfch.specmgr.tsk.models.v1.Task`/
    :class:`~biz.dfch.specmgr.tsk.models.v1.TskFrontmatter`/
    :class:`~biz.dfch.specmgr.tsk.models.v1.TskDocument`'s own Pydantic
    ...
    exactly as ``create_tsk``/``update_tsk`` themselves do.
