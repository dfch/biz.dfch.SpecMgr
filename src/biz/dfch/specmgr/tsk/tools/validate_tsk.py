...
ever calling ``create_tsk`` or the generic ``update`` tool in
...
argument, exposed standalone here. Mirrors
...
def validate_tsk(content: str, full: bool = False) -> bool:
    ...
    exactly as ``create_tsk`` and the generic ``update`` tool do.
    ...
    full:
        ...
        shape ``create_tsk`` and the generic ``update`` tool accept) --
        raises ``ValueError``
