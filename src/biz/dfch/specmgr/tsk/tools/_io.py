...
**The second, independent ``read_tsk`` call is also guarded against a
vanished file (feat-107-doc-cache Phase 8, REQ-015).** :func:`load_by_id`
calls ``read_tsk(path)`` again immediately after ``find_tsk_path`` already
...
per-id lock, never a whole-domain lock, while ``get_tsk``/``list_tsk``
...
``find_tsk_path``'s own read and this one, raising ``FileNotFoundError``.
...
translated into :class:`._paths.TskNotFoundError` here, rather than left to
propagate uncaught.
"""
...
from ._cache import read_tsk
from ._paths import TskNotFoundError, find_tsk_path
...
def load_by_id(base_dir: Path, id_: str) -> tuple[Path, TskDocument]:
    ...
    TskNotFoundError
        ...
        :func:`._paths.find_tsk_path` an instant earlier -- fails this
        function's own second, independent :func:`._cache.read_tsk` call
    ...
    try:
        ...
        raise TskNotFoundError(
            f"task list {id_!r} exists at {path}, but its content could not be read as a valid "
            f"task list document on this second read ({type(ex).__name__}: {ex})."
        ) from ex
    result = (path, doc)
    return result
