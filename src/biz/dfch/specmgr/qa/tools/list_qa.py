...
from ..models.v2 import QaDocument, QaSummary
from ._cache import reconcile_qa_cache
from ._io import read_qa
from ._paths import iter_qa_paths


def _to_summary(doc: QaDocument, path: Path) -> QaSummary:
    result = QaSummary(
...
def _to_failed_summary(path: Path, error: Exception) -> QaSummary:
    result = default_failed_summary(QaSummary, path, error)
    return result
...
def list_qa(max_results: int | None = None, offset: int | None = None) -> PagedResult[QaSummary]:
    ...
    reconcile_qa_cache(paths)  # feat-107-doc-cache Phase 4, REQ-005
    summaries, error_count = build_summaries(paths, read_qa, _to_summary, _to_failed_summary)
