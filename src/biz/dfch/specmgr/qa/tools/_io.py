...
from ._cache import read_qa
from ._paths import QaNotFoundError, find_qa_path
...
def load_by_id(base_dir: Path, id_: str) -> tuple[Path, QaDocument]:
    ...
    try:
        ...
        raise QaNotFoundError(
            f"QA document {id_!r} exists at {path}, but its content could not be read as a valid "
            f"QA document on this second read ({type(ex).__name__}: {ex})."
        ) from ex
    result = (path, doc)
    return result
