...
#: ``channel`` value for a call site that only ever validates body-only content (no frontmatter
#: block can be present, by construction of the tool's own contract).
BODY_CHANNEL = "body"

#: ``channel`` value for a call site that only ever validates a frontmatter field/value (the
#: YAML syntax itself is not in play -- e.g. ``set_status``'s ``XFrontmatter(**fm_data)`` retry).
FRONTMATTER_CHANNEL = "frontmatter"

#: The ``pydantic_core`` error "type" tag used for every message this module re-raises with.
#: Not one of pydantic's own recognized built-in types (mirrors ``_frontmatter_parse.py``'s own
#: ``_ENRICHED_ERROR_TYPE``), so rendering never appends an unrelated
#: "For further information visit https://errors.pydantic.dev/..." URL.
_ENRICHED_ERROR_TYPE = "tool_boundary_error"


def _context_label(domain: str, tool: str, channel: str | None) -> str:
    """Build the ``"{domain} {tool}"`` (optionally ``" ({channel})"``) prefix label."""
    assert isinstance(domain, str) and domain.strip(), domain
    assert isinstance(tool, str) and tool.strip(), tool
    assert channel is None or (isinstance(channel, str) and channel.strip()), channel

    result = f"{domain} {tool}"
    if channel is not None:
        result += f" ({channel})"
    return result


def _reraise_assertion_like(error: Exception, label: str) -> Exception:
    ...
    """
    result = type(error)(f"{label}: {error}")
    return result


def _reraise_validation_error(error: ValidationError, label: str) -> ValidationError:
    ...
    line_errors: list[InitErrorDetails] = [
        InitErrorDetails(
            type=PydanticCustomError(_ENRICHED_ERROR_TYPE, f"{label}: {detail['msg']}"),
            loc=detail["loc"],
            input=detail["input"],
        )
...
def _reraise_yaml_error(error: yaml.YAMLError, label: str) -> yaml.YAMLError:
    ...
    context = f"{label}: {error.context}" if error.context else label
    result = type(error)(
        context=context,
...
def wrap_tool_errors(
    ...
    *,
    ...
    assert isinstance(domain, str) and domain.strip(), domain
    assert isinstance(tool, str) and tool.strip(), tool
    assert isinstance(also_catch, tuple), type(also_catch)

    label = _context_label(domain, tool, channel)
    try:
        yield
    except ValidationError as error:
        raise _reraise_validation_error(error, label) from error
    except yaml.YAMLError as error:
        raise _reraise_yaml_error(error, label) from error
    except (AssertionError, *also_catch) as error:
        raise _reraise_assertion_like(error, label) from error
