...
def parse_req(text: str) -> ReqDocument:
    ...
    """
    fm, content = parse_frontmatter(text, ReqFrontmatter, domain="req", stringify_metadata=_stringify_metadata)
...
def _stringify_metadata(metadata: dict[str, object]) -> dict[str, object]:
    ...
    ``date`` objects, but every :class:`ReqFrontmatter` field inherited from
