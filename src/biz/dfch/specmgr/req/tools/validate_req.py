...
draft before ever calling ``create_req``/``update_req`` (or independently of
...
from ..models.v1 import Requirement, parse_req


@mcp.tool(
    name="validate_req",
    title="Validate requirement content",
    description=(
        "Disk-free, id-free dry run validating requirement content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_req(content: str, full: bool = False) -> bool:
    ...
    exactly as ``create_req``/``update_req`` themselves do.
    ...
    full:
        ...
        shape ``create_req``/``update_req`` accept) -- raises ``ValueError``
