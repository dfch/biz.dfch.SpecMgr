...
``update_req`` (Task 3.13) and any future requirement mutation tool
(``set_status_req``, Task 3.14) wrap their whole sequence in
``with req_lock(id):``.
...
#: One lock per requirement id, created lazily on first use and never
...
@contextmanager
def req_lock(id_: str) -> Iterator[None]:
