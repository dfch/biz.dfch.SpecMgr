...
#: The packaged, build-guaranteed copy of REQ's generated JSON Schema
#: (Task 3.8). Kept in sync with ``docs/req_schema.json`` by a dedicated
#: pre-commit hook/CI step that runs
#: ``specmgr schema --type req --output-dir src/biz/dfch/specmgr/req/resources/data``
#: -- the same generator, just a second ``--output-dir``, so no bespoke
#: copy logic was added to ``commands/schema.py``. Same patchable
#: module-level constant shape as ``_EXAMPLE_PATH``/``_TEMPLATE_PATH``.
_SCHEMA_PATH: Traversable = resources.files(_DATA_PACKAGE) / "data" / "req_schema.json"
...
def read_req_schema_text() -> str:
    ...
    FileNotFoundError
        ...
        a broken installation).
    """
    result: str = _SCHEMA_PATH.read_text(encoding="utf-8")
    return result
