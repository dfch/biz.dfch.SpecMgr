...
from ..models.v1 import Sop, parse_sop


@mcp.tool(
    name="validate_sop",
    title="Validate SOP content",
    description=(
        "Disk-free, id-free dry run validating SOP content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_sop(content: str, full: bool = False) -> bool:
    ...
    if full:
        ...
        with wrap_tool_errors(domain="sop", tool="validate_sop"):
            parse_sop(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="sop", tool="validate_sop", channel=BODY_CHANNEL):
