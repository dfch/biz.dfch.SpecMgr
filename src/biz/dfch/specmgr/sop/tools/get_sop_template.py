...
@mcp.tool(
    name="get_sop_template",
    title="Get SOP template",
    description=(
        "Return a SOP document template -- frontmatter and every body field present, populated "
        "with short placeholder ('blind text') content -- as raw markdown, for use as a starting "
        "point when drafting a new Standard Operating Procedure."
    ),
)
def get_sop_template() -> str:
    ...
    str
        ...
        frontmatter block.
    """
    return read_packaged_text("sop", "template")
