...
@mcp.tool(
    name="get_sop",
    title="Get Standard Operating Procedure",
    description=(
        "Read, parse, and return a full SOP document (frontmatter and body) by its id. "
...
def get_sop(id: str, raw: bool = False) -> SopDocument | str:
    ...
    if raw:
        result: SopDocument | str = body_text(path)
        return result
    result = doc
    return result
