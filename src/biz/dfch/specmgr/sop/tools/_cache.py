...
"""Per-domain content-hash-validated read cache singleton for SOPs (feat-107-doc-cache Phase 4).

1:1 port of ``req.tools._cache``'s shape (feat-107-doc-cache Phase 3, the
pilot domain) for the ``sop`` domain -- see that module's own docstring for
the full circular-import rationale this module also follows: ``sop.tools._io``'s
``load_by_id`` needs ``find_sop_path`` (from ``_paths.py``) to resolve an id to
a path; with the cache wired in, ``_paths.py``'s ``find_sop_path`` needs a
cache-backed ``read_fn`` to pass to
``general.tools._doc_paths.find_doc_path_by_id`` -- and the only cache-backed
reader available is ``read_sop``. Pulling the cache singleton and ``read_sop``
...
``sop.models.v1`` (SopDocument/parse_sop).
...
(``read_sop``/``invalidate_sop_cache``/``reconcile_sop_cache``), and those
...
:func:`reset_sop_cache` exists purely so tests can clear every cached entry
...
module wires up for the ``sop`` domain, and
...
__all__ = ["invalidate_sop_cache", "read_sop", "reconcile_sop_cache", "reset_sop_cache"]
...
def _parse(path: Path) -> SopDocument:
    """Read and parse ``path``'s full text into a :class:`SopDocument` (the cache's own ``parse_fn``)."""
    assert isinstance(path, Path), type(path)

    result = parse_sop(path.read_text(encoding="utf-8"))
    return result


def read_sop(path: Path) -> SopDocument:
    """Read and parse the SOP at ``path``, through the cache.
    ...
    :func:`~biz.dfch.specmgr.sop.models.v1.parse_sop`.

    Parameters
    ----------
    path:
        The filesystem path to the SOP ``.md`` file.
...
def invalidate_sop_cache(path: Path) -> None:
    """Drop ``path``'s cached entry, if present.

    Called by the generic ``delete`` tool's ``sop`` adapter immediately
...
def reconcile_sop_cache(live_paths: Iterable[Path]) -> None:
    """Drop every cached entry whose path is not in ``live_paths`` (REQ-005).

    Called before any per-file work in both ``find_sop_path``'s scan (via
    ``general.tools._doc_paths.find_doc_path_by_id``'s ``reconcile_fn``
    parameter) and ``list_sop``, so a file deleted outside specmgr's own
    tooling does not leak in memory indefinitely.

    Parameters
    ----------
    live_paths:
        The current, live set of on-disk paths for the ``sop`` domain.
    """
    _cache.reconcile(live_paths)


def reset_sop_cache() -> None:
