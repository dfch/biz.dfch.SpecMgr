...
from ..models.v1 import SopDocument, SopSummary
from ._cache import reconcile_sop_cache
from ._io import read_sop
from ._paths import iter_sop_paths


def _to_summary(doc: SopDocument, path: Path) -> SopSummary:
    result = SopSummary(
...
def _to_failed_summary(path: Path, error: Exception) -> SopSummary:
    result = default_failed_summary(SopSummary, path, error)
    return result


@mcp.tool(
    name="list_sop",
    title="List Standard Operating Procedures",
    description=(
        "Ids, titles, statuses, and refs of Standard Operating Procedures in the configured SOP base directory, "
        ...
        "have no assigned id; use it with the get_sop tool instead. max_results/offset control "
        "paging (default page size 25, capped at 100); out-of-range values are clamped, not errored."
    ),
)
def list_sop(max_results: int | None = None, offset: int | None = None) -> PagedResult[SopSummary]:
    ...
    :func:`~biz.dfch.specmgr.sop.models.v1.parse_sop` raises) appears inline
    ...
    PagedResult[SopSummary]
        ...
        if the base directory does not exist, holds no SOPs, or ``offset``
        is past the end of the full list.
    """
    paths = list(iter_sop_paths())
    reconcile_sop_cache(paths)  # feat-107-doc-cache Phase 4, REQ-005
    summaries, error_count = build_summaries(paths, read_sop, _to_summary, _to_failed_summary)
