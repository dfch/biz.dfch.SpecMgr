...
**The second, independent ``read_sop`` call is also guarded against a
vanished file (feat-107-doc-cache Phase 8, REQ-015).** :func:`load_by_id`
calls ``read_sop(path)`` again immediately after ``find_sop_path`` already
...
per-id lock, never a whole-domain lock, while ``get_sop``/``list_sop``
...
``find_sop_path``'s own read and this one, raising ``FileNotFoundError``.
...
translated into :class:`._paths.SopNotFoundError` here, rather than left to
propagate uncaught.
"""
...
from ._paths import SopNotFoundError, find_sop_path
...
def load_by_id(base_dir: Path, id_: str) -> tuple[Path, SopDocument]:
    ...
    SopNotFoundError
        ...
        :func:`._paths.find_sop_path` an instant earlier -- fails this
        function's own second, independent :func:`._cache.read_sop` call
    ...
    try:
        ...
        raise SopNotFoundError(
            f"SOP {id_!r} exists at {path}, but its content could not be read as a valid "
            f"SOP document on this second read ({type(ex).__name__}: {ex})."
        ) from ex
    result = (path, doc)
    return result
