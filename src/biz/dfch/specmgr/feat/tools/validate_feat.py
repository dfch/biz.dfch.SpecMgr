...
from ..models.v1 import Feature, parse_feat


@mcp.tool(
    name="validate_feat",
    title="Validate feature content",
    description=(
        "Disk-free, id-free dry run validating feature content. `full=False` (default) validates "
        "body-only content (no frontmatter); `full=True` validates a complete document (frontmatter "
        "+ body)."
    ),
)
def validate_feat(content: str, full: bool = False) -> bool:
    ...
    if full:
        ...
        with wrap_tool_errors(domain="feat", tool="validate_feat"):
            parse_feat(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="feat", tool="validate_feat", channel=BODY_CHANNEL):
