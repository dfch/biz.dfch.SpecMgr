...
from ._domains import WHOLE_BODY_DOMAINS, whole_body_domain
from ._listing import DEFAULT_ERROR_TYPES
from ._path_safety import assert_within, validate_id
from ._similarity_text import (
    SimilarityText,
...
def iter_candidate_paths(target_types: Sequence[str] | None = None) -> Iterator[tuple[str, Path]]:
    ...
    """
    assert target_types is None or (
        isinstance(target_types, Sequence) and not isinstance(target_types, (str, bytes))
    ), type(target_types)

    names = _validated_target_types(target_types)
    result = _iter_candidate_paths(names)
    return result


def _validated_target_types(target_types: Sequence[str] | None) -> tuple[str, ...]:
    ...
    if target_types is None:
        result = WHOLE_BODY_DOMAINS
        return result

    validated: list[str] = []
    for name in target_types:
        whole_body_domain(name)  # ValueError (unknown/adr) before any filesystem access
...
def _iter_candidate_paths(names: tuple[str, ...]) -> Iterator[tuple[str, Path]]:
    """The actual lazy walk over the validated domain names (see :func:`iter_candidate_paths`)."""
    assert isinstance(names, tuple), type(names)

    for name in names:
        domain = whole_body_domain(name)
        base_dir = domain.base_dir()
        for path in domain.iter_paths(base_dir):
            yield name, path


def resolve_source(type_: str, id_: str) -> tuple[str, Path, Any]:
    ...
    domain = whole_body_domain(type_)  # ValueError (unknown/adr) before any filesystem access
    validate_id(type_, id_)  # ValueError (injection/wrong format) before any filesystem access
    base_dir = domain.base_dir()
    path, doc = domain.load_by_id(base_dir, id_)  # the domain's own XNotFoundError propagates unchanged
    assert_within(base_dir, path)  # defense-in-depth confinement
    result = (type_, path, doc)
    return result


def is_parseable(domain: str, text: str) -> bool:
    ...
    parse_fn = whole_body_domain(domain).parse_text
    try:
        parse_fn(text)
    except DEFAULT_ERROR_TYPES:
        return False
    return True


def candidate_similarity_text(domain: str, text: str) -> SimilarityText:
    ...
    parse_fn = whole_body_domain(domain).parse_text
    try:
        doc: Any = parse_fn(text)
    except DEFAULT_ERROR_TYPES:
        result = failed_similarity_text(text)
        return result
