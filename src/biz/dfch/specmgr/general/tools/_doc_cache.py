    ...
    *different* paths never block each other on that bookkeeping. A
    ...
    def read(self, path: Path, parse_fn: Callable[[str], _DocT]) -> _DocT:
        ...
        try:
            ...
            with self._lock:
                self._entries[resolved_path] = (content_hash, exc)
            raise

        with self._lock:
            self._entries[resolved_path] = (content_hash, result)
        return result
    ...
    def reconcile(self, live_paths: Iterable[Path]) -> None:
        ...
        """
        live_set = {live_path.resolve() for live_path in live_paths}
    ...
    def move(self, old_path: Path, new_path: Path) -> None:
        ...
        *never* byte-for-byte identical in the realistic case. The moved
        ...
        with self._lock:
            entry = self._entries.pop(resolved_old_path, None)
            if entry is not None:
                self._entries[resolved_new_path] = entry
