...
"""Resource: specmgr://vcr/example (Task 3.1).

Read-only, addressable counterpart of the ``get_vcr_example`` tool, mirroring
...
``vcr.tools`` (nor vice versa): both this resource and the ``get_vcr_example``
...
``specmgr://vcr/schema``'s own precedent.
"""
...
@mcp.resource(
    "specmgr://vcr/example",
    name="vcr_example",
    title="Verification Case Record (VCR) Example",
    description=(
        "A complete, valid sample verification case record document as raw markdown -- "
...
def vcr_example() -> str:
    ...
    design as ``vcr.tools.get_vcr_example.get_vcr_example`` -- this is simply
    ...
    str
        ...
        frontmatter block.
    """
    return read_packaged_text("vcr", "example")
