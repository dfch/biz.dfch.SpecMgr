...
from ..models.v1 import VcrSummary
...
def list_vcr(max_results: int | None = None, offset: int | None = None) -> PagedResult[VcrSummary]:
    ...
    :func:`~biz.dfch.specmgr.vcr.models.v1.parse_vcr` raises) is silently
    skipped -- a single malformed file must not break listing every other
    valid one (mirrors ``vcr.tools._paths.find_vcr_path``'s own
    ...
    PagedResult[VcrSummary]
        ...
        the base directory does not exist, holds no verification case
        records, or ``offset`` is past the end of the full list.
    """
    summaries: list[VcrSummary] = []
    for path in iter_vcr_paths():
        try:
            doc = read_vcr(path)
