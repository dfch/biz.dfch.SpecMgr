...
from ..models.v1 import VcrDocument, VcrSummary
from ._cache import reconcile_vcr_cache
from ._io import read_vcr
from ._paths import iter_vcr_paths


def _to_summary(doc: VcrDocument, path: Path) -> VcrSummary:
    result = VcrSummary(
...
def _to_failed_summary(path: Path, error: Exception) -> VcrSummary:
    result = default_failed_summary(VcrSummary, path, error)
    return result


@mcp.tool(
    name="list_vcr",
    title="List verification case records",
    description=(
        "Ids, titles, statuses, and refs of verification case records in the configured "
        "verification case record base directory, one page at a time, for context before "
        "addressing one by id. 'ref' is an opaque, extensionless identifier -- not a filename to "
        "read from disk -- for documents that have no assigned id; use it with the get_vcr tool "
        "instead. max_results/offset control paging (default page size 25, capped at 100); "
        "out-of-range values are clamped, not errored."
    ),
)
def list_vcr(max_results: int | None = None, offset: int | None = None) -> PagedResult[VcrSummary]:
    ...
    PagedResult[VcrSummary]
        ...
        if the base directory does not exist, holds no verification case
        records, or ``offset`` is past the end of the full list.
    """
    paths = list(iter_vcr_paths())
    reconcile_vcr_cache(paths)  # feat-107-doc-cache Phase 4, REQ-005
    summaries, error_count = build_summaries(paths, read_vcr, _to_summary, _to_failed_summary)
