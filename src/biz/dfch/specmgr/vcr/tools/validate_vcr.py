...
"""``@mcp.tool()`` wrapper: validate_vcr (Task 2.1).

Unlike ``validate_adr`` (which is id-based and re-reads a file from disk),
``validate_vcr`` is a **disk-free, id-free dry run**: it validates a
submitted ``content`` string directly, without ever touching the
verification case record base directory or resolving an id. This lets a
caller check a draft before ever calling ``create_vcr`` or the generic
...
from ..models.v1 import Vcr, parse_vcr


@mcp.tool(
    name="validate_vcr",
    title="Validate verification case record content",
    description=(
        "Disk-free, id-free dry run validating verification case record content. `full=False` "
        "(default) validates body-only content (no frontmatter); `full=True` validates a complete "
        "document (frontmatter + body)."
    ),
)
def validate_vcr(content: str, full: bool = False) -> bool:
    """Validate ``content`` as verification case record markdown, without reading or writing any file.

    "Validate" means letting :class:`~biz.dfch.specmgr.vcr.models.v1.Vcr`/
    :class:`~biz.dfch.specmgr.vcr.models.v1.VcrFrontmatter`/
    :class:`~biz.dfch.specmgr.vcr.models.v1.VcrDocument`'s own Pydantic
    ...
    exactly as ``create_vcr`` and the generic ``update`` tool do.
    ...
    if full:
        ...
        with wrap_tool_errors(domain="vcr", tool="validate_vcr"):
            parse_vcr(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="vcr", tool="validate_vcr", channel=BODY_CHANNEL):
