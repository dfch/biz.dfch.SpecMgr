...
@mcp.prompt(
    name="update_sysrs",
    title="Update a system requirements specification",
    description=(
        "Guides the LLM through revising an existing SYSRS by id: reading current "
        "state, applying the requested change with the right tool, and validating."
    ),
)
def update_sysrs(id: str, instructions: str | None = None) -> str:
    ...
    """
    template = Template(read_packaged_text("sysrs", "update_instructions", "md"))
