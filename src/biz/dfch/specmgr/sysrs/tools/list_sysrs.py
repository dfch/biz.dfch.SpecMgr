...
def _to_summary(doc: SysrsDocument, path: Path) -> SysrsSummary:
    result = SysrsSummary(
...
def _to_failed_summary(path: Path, error: Exception) -> SysrsSummary:
    result = default_failed_summary(SysrsSummary, path, error)
    return result
