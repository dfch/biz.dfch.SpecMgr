...
"""Per-domain content-hash-validated read cache singleton for SYSRS documents (feat-107-doc-cache Phase 4).

1:1 port of ``req.tools._cache``'s shape (feat-107-doc-cache Phase 3, the
pilot domain) for the ``sysrs`` domain -- see that module's own docstring for
the full circular-import rationale this module also follows: ``sysrs.tools._io``'s
``load_by_id`` needs ``find_sysrs_path`` (from ``_paths.py``) to resolve an id to
a path; with the cache wired in, ``_paths.py``'s ``find_sysrs_path`` needs a
cache-backed ``read_fn`` to pass to
``general.tools._doc_paths.find_doc_path_by_id`` -- and the only cache-backed
reader available is ``read_sysrs``. Pulling the cache singleton and ``read_sysrs``
...
``sysrs.models.v1`` (SysrsDocument/parse_sysrs).
...
(``read_sysrs``/``invalidate_sysrs_cache``/``reconcile_sysrs_cache``), and those
...
:func:`reset_sysrs_cache` exists purely so tests can clear every cached entry
...
module wires up for the ``sysrs`` domain, and
...
__all__ = ["invalidate_sysrs_cache", "read_sysrs", "reconcile_sysrs_cache", "reset_sysrs_cache"]
...
def _parse(path: Path) -> SysrsDocument:
    """Read and parse ``path``'s full text into a :class:`SysrsDocument` (the cache's own ``parse_fn``)."""
...
def read_sysrs(path: Path) -> SysrsDocument:
    """Read and parse the System Requirements Specification at ``path``, through the cache.
    ...
    :func:`~biz.dfch.specmgr.sysrs.models.v1.parse_sysrs`.
...
def invalidate_sysrs_cache(path: Path) -> None:
    """Drop ``path``'s cached entry, if present.

    Called by the generic ``delete`` tool's ``sysrs`` adapter immediately
...
def reconcile_sysrs_cache(live_paths: Iterable[Path]) -> None:
    """Drop every cached entry whose path is not in ``live_paths`` (REQ-005).

    Called before any per-file work in both ``find_sysrs_path``'s scan (via
    ``general.tools._doc_paths.find_doc_path_by_id``'s ``reconcile_fn``
    parameter) and ``list_sysrs``, so a file deleted outside specmgr's own
    tooling does not leak in memory indefinitely.

    Parameters
    ----------
    live_paths:
        The current, live set of on-disk paths for the ``sysrs`` domain.
    """
    _cache.reconcile(live_paths)


def reset_sysrs_cache() -> None:
