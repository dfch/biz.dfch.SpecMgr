...
"""``@mcp.tool()`` wrapper: parse_sysrs (Task 3.2).

Reads a System Requirements Specification markdown file from disk and
parses it into a structured :class:`SysrsDocument`, mirroring
``vcr.tools.parse_vcr``/``dec.tools.parse_dec``'s own pattern -- read path
-> parse via free-function returning typed document model. Errors
...
@mcp.tool(
    name="parse_sysrs",
    title="Parse System Requirements Specification",
    description=(
        "Parse a System Requirements Specification markdown file (YAML frontmatter + body) from "
...
def parse_sysrs(path: str) -> SysrsDocument:
    ...
    here also means "validate": letting :class:`Sysrs` /
    :class:`SysrsFrontmatter` / :class:`SysrsDocument`'s own Pydantic
    validators run during parsing is the only validation pass there is --
    there is no separate validation step. Any structural problem (unrecognized/misplaced
    ...
    AssertionError
        ...
        ``"sysrs parse_sysrs: ..."``) by the shared tool-boundary wrapper
