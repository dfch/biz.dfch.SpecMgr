...
"""``@mcp.tool()`` wrapper: validate_sysrs (Task 3.2).

Unlike ``validate_adr`` (which is id-based and re-reads a file from disk),
``validate_sysrs`` is a **disk-free, id-free dry run**: it validates a
submitted ``content`` string directly, without ever touching the System
Requirements Specification base directory or resolving an id. This lets a
caller check a draft before ever calling ``create_sysrs`` or the generic
``update`` tool in ``general.tools`` (or independently of either), and is
...
from ..models.v1 import Sysrs, parse_sysrs


@mcp.tool(
    name="validate_sysrs",
    title="Validate System Requirements Specification content",
    description=(
        "Disk-free, id-free dry run validating System Requirements Specification content. "
        "`full=False` (default) validates body-only content (no frontmatter); `full=True` "
        "validates a complete document (frontmatter + body)."
    ),
)
def validate_sysrs(content: str, full: bool = False) -> bool:
    """Validate ``content`` as System Requirements Specification markdown, without reading or writing any file.

    "Validate" means letting :class:`~biz.dfch.specmgr.sysrs.models.v1.Sysrs`/
    :class:`~biz.dfch.specmgr.sysrs.models.v1.SysrsFrontmatter`/
    :class:`~biz.dfch.specmgr.sysrs.models.v1.SysrsDocument`'s own Pydantic
    ...
    exactly as ``create_sysrs`` and the generic ``update`` tool do.
    ...
    ----------
    content:
        The System Requirements Specification markdown to validate.
    full:
        ``False`` (default): ``content`` must be body markdown only (the
        shape ``create_sysrs`` and the generic ``update`` tool accept) --
        raises ``ValueError``
        if a frontmatter block is found instead. ``True``: ``content`` must
        be a complete document, frontmatter and body together (the shape
        ``parse_sysrs`` expects for an on-disk file) -- raises the symmetric
    ...
    if full:
        ...
        with wrap_tool_errors(domain="sysrs", tool="validate_sysrs"):
            parse_sysrs(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="sysrs", tool="validate_sysrs", channel=BODY_CHANNEL):
