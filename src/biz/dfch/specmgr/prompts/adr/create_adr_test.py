...
``prompts/adr/create_adr.py``), kept as a *separate* prompt -- not a
replacement -- so the two can be registered side by side and compared: the
same underlying MADR structure and ``tools/adr/`` sequence, but rewritten
...
collide with any ``@mcp.tool()`` -- ``tools/adr/`` has no ``create_adr_test``
...
def create_adr_test(
    ...
    See ``create_adr`` (``prompts/adr/create_adr.py``) for the non-gated
