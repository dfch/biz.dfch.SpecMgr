...
"""Per-domain content-hash-validated read cache singleton for goals (feat-107-doc-cache Phase 4).

1:1 port of ``req.tools._cache``'s shape (feat-107-doc-cache Phase 3, the
pilot domain) for the ``gol`` domain -- see that module's own docstring for
the full circular-import rationale this module also follows: ``gol.tools._io``'s
``load_by_id`` needs ``find_gol_path`` (from ``_paths.py``) to resolve an id to
a path; with the cache wired in, ``_paths.py``'s ``find_gol_path`` needs a
cache-backed ``read_fn`` to pass to
``general.tools._doc_paths.find_doc_path_by_id`` -- and the only cache-backed
reader available is ``read_gol``. Pulling the cache singleton and ``read_gol``
...
``gol.models.v1`` (GolDocument/parse_gol).
...
(``read_gol``/``invalidate_gol_cache``/``reconcile_gol_cache``), and those
...
:func:`reset_gol_cache` exists purely so tests can clear every cached entry
...
module wires up for the ``gol`` domain, and
...
__all__ = ["invalidate_gol_cache", "read_gol", "reconcile_gol_cache", "reset_gol_cache"]
...
def read_gol(path: Path) -> GolDocument:
    """Read and parse the goal at ``path``, through the cache.
    ...
    :func:`~biz.dfch.specmgr.gol.models.v1.parse_gol`.
...
def invalidate_gol_cache(path: Path) -> None:
    """Drop ``path``'s cached entry, if present.

    Called by the generic ``delete`` tool's ``gol`` adapter immediately
...
def reconcile_gol_cache(live_paths: Iterable[Path]) -> None:
    """Drop every cached entry whose path is not in ``live_paths`` (REQ-005).

    Called before any per-file work in both ``find_gol_path``'s scan (via
    ``general.tools._doc_paths.find_doc_path_by_id``'s ``reconcile_fn``
    parameter) and ``list_gol``, so a file deleted outside specmgr's own
    tooling does not leak in memory indefinitely.

    Parameters
    ----------
    live_paths:
        The current, live set of on-disk paths for the ``gol`` domain.
    """
    _cache.reconcile(live_paths)


def reset_gol_cache() -> None:
