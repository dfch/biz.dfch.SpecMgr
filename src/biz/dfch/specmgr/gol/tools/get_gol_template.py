...
@mcp.tool(
    name="get_gol_template",
    title="Get GOL template",
    description=(
        "Return a GOL document template -- frontmatter and every body field present, populated "
        "with short placeholder ('blind text') content -- as raw markdown, for use as a starting "
        "point when drafting a new goal."
    ),
)
def get_gol_template() -> str:
    ...
    str
        ...
        frontmatter block.
    """
    return read_packaged_text("gol", "template")
