...
effect), only :func:`ensure_gol_base_dir` does, for ``create_gol``.

**Cache-backed scan (feat-107-doc-cache Phase 4).** :func:`find_gol_path`
...
``read_gol`` (not ``parse_gol``) as ``find_doc_path_by_id``'s ``read_fn``,
so a file whose on-disk content hash is unchanged since its last read is
not re-parsed, and it passes ``._cache``'s ``reconcile_gol_cache`` as
...
``read_gol``/``reconcile_gol_cache`` are imported from ``._cache``, not
...
from ._cache import read_gol, reconcile_gol_cache
...
def find_gol_path(base_dir: Path, id_: str) -> Path:
    ...
    cache-backed :func:`~._cache.read_gol` (feat-107-doc-cache Phase 4) and
    ...
    (:func:`~._cache.reconcile_gol_cache`), dropping any cached entry for a
    ...
    try:
        result = find_doc_path_by_id(base_dir, id_, read_gol, _get_gol_id, reconcile_fn=reconcile_gol_cache)
