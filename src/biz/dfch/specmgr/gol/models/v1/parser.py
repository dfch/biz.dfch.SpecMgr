...
"""Parse raw goal (GOL) ``.md`` text into a :class:`GolDocument` (Phase 2, Task 2.2).

Fills the ``from_text``/parser entry-point gap ``document.py``'s own docstring
flags: ``GolDocument`` deliberately holds no such method itself, and the generic
``models/md`` engine only ever parses a *body* (``Goal.from_text``), never the
...
``req/models/v1/parser.parse_req``/``prb/models/v1/parser.parse_prb``'s own
...
Like ``req.models.v1.parser.parse_req``/``prb.models.v1.parser.parse_prb``,
there is no dedicated structural-error exception type (no ``GolParseError``
...
def parse_gol(text: str) -> GolDocument:
    """Parse a full goal ``.md`` file's text into a :class:`GolDocument`.
...
def _stringify_metadata(metadata: dict[str, object]) -> dict[str, object]:
    ...
    ``date`` objects, but every :class:`GolFrontmatter` field inherited from
    ...
    ``prb/models/v1/parser._stringify_metadata``.
    """
