...
def parse_prb(text: str) -> PrbDocument:
    ...
    """
    fm, content = parse_frontmatter(text, PrbFrontmatter, domain="prb", stringify_metadata=_stringify_metadata)
    body = Prb.from_text(format_text(content))
    return PrbDocument(frontmatter=fm, body=body)
...
def _stringify_metadata(metadata: dict[str, object]) -> dict[str, object]:
    ...
    ``date`` objects, but every :class:`PrbFrontmatter` field inherited from
