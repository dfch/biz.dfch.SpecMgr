...
"""Per-domain content-hash-validated read cache singleton for problem statements (feat-107-doc-cache Phase 4).

1:1 port of ``req.tools._cache``'s shape (feat-107-doc-cache Phase 3, the
pilot domain) for the ``prb`` domain -- see that module's own docstring for
the full circular-import rationale this module also follows: ``prb.tools._io``'s
``load_by_id`` needs ``find_prb_path`` (from ``_paths.py``) to resolve an id to
a path; with the cache wired in, ``_paths.py``'s ``find_prb_path`` needs a
cache-backed ``read_fn`` to pass to
``general.tools._doc_paths.find_doc_path_by_id`` -- and the only cache-backed
reader available is ``read_prb``. Pulling the cache singleton and ``read_prb``
...
``prb.models.v1`` (PrbDocument/parse_prb).
...
(``read_prb``/``invalidate_prb_cache``/``reconcile_prb_cache``), and those
...
:func:`reset_prb_cache` exists purely so tests can clear every cached entry
...
module wires up for the ``prb`` domain, and
...
__all__ = ["invalidate_prb_cache", "read_prb", "reconcile_prb_cache", "reset_prb_cache"]
...
def read_prb(path: Path) -> PrbDocument:
    """Read and parse the problem statement at ``path``, through the cache.
    ...
    :func:`~biz.dfch.specmgr.prb.models.v1.parse_prb`.
...
def invalidate_prb_cache(path: Path) -> None:
    """Drop ``path``'s cached entry, if present.

    Called by the generic ``delete`` tool's ``prb`` adapter immediately
...
def reconcile_prb_cache(live_paths: Iterable[Path]) -> None:
    """Drop every cached entry whose path is not in ``live_paths`` (REQ-005).

    Called before any per-file work in both ``find_prb_path``'s scan (via
    ``general.tools._doc_paths.find_doc_path_by_id``'s ``reconcile_fn``
    parameter) and ``list_prb``, so a file deleted outside specmgr's own
    tooling does not leak in memory indefinitely.

    Parameters
    ----------
    live_paths:
        The current, live set of on-disk paths for the ``prb`` domain.
    """
    _cache.reconcile(live_paths)


def reset_prb_cache() -> None:
