...
``update_prb``/``set_status_prb`` wrap their whole sequence in
``with prb_lock(id):``.
...
#: One lock per problem statement id, created lazily on first use and never
...
@contextmanager
def prb_lock(id_: str) -> Iterator[None]:
