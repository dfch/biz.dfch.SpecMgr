...
**The second, independent ``read_prb`` call is also guarded against a
vanished file (feat-107-doc-cache Phase 8, REQ-015).** :func:`load_by_id`
calls ``read_prb(path)`` again immediately after ``find_prb_path`` already
...
per-id lock, never a whole-domain lock, while ``get_prb``/``list_prb``
...
``find_prb_path``'s own read and this one, raising ``FileNotFoundError``.
...
translated into :class:`._paths.PrbNotFoundError` here, rather than left to
propagate uncaught.
"""
...
from ._cache import read_prb
from ._paths import PrbNotFoundError, find_prb_path
...
def load_by_id(base_dir: Path, id_: str) -> tuple[Path, PrbDocument]:
    ...
    PrbNotFoundError
        ...
        :func:`._paths.find_prb_path` an instant earlier -- fails this
        function's own second, independent :func:`._cache.read_prb` call
    ...
    try:
        ...
        raise PrbNotFoundError(
            f"problem statement {id_!r} exists at {path}, but its content could not be read as a valid "
            f"problem statement document on this second read ({type(ex).__name__}: {ex})."
        ) from ex
    result = (path, doc)
    return result
