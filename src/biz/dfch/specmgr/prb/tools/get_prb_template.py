...
@mcp.tool(
    name="get_prb_template",
    title="Get PRB template",
    description=(
        "Return a PRB document template -- frontmatter and every body field present, populated "
        "with short placeholder ('blind text') content -- as raw markdown, for use as a starting "
        "point when drafting a new problem statement."
    ),
)
def get_prb_template() -> str:
    ...
    str
        ...
        frontmatter block.
    """
    return read_packaged_text("prb", "template")
