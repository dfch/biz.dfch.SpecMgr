...
effect), only :func:`ensure_prb_base_dir` does, for ``create_prb``.

**Cache-backed scan (feat-107-doc-cache Phase 4).** :func:`find_prb_path`
...
``read_prb`` (not ``parse_prb``) as ``find_doc_path_by_id``'s ``read_fn``,
so a file whose on-disk content hash is unchanged since its last read is
not re-parsed, and it passes ``._cache``'s ``reconcile_prb_cache`` as
...
``read_prb``/``reconcile_prb_cache`` are imported from ``._cache``, not
...
from ._cache import read_prb, reconcile_prb_cache
...
def find_prb_path(base_dir: Path, id_: str) -> Path:
    ...
    cache-backed :func:`~._cache.read_prb` (feat-107-doc-cache Phase 4) and
    ...
    (:func:`~._cache.reconcile_prb_cache`), dropping any cached entry for a
    ...
    try:
        result = find_doc_path_by_id(base_dir, id_, read_prb, _get_prb_id, reconcile_fn=reconcile_prb_cache)
