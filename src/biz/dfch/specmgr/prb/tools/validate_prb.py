...
from ..models.v1 import Prb, parse_prb


@mcp.tool(
    name="validate_prb",
    title="Validate problem statement content",
    description=(
        "Disk-free, id-free dry run validating problem statement content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_prb(content: str, full: bool = False) -> bool:
    ...
    if full:
        ...
        with wrap_tool_errors(domain="prb", tool="validate_prb"):
            parse_prb(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="prb", tool="validate_prb", channel=BODY_CHANNEL):
