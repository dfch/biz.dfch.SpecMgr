...
- A change to the body -- the `cause`, `trigger`, `consequence`,
  ...
  - **Line-range replace** (a localized change -- one paragraph, field,
    or section): first call `get_rsk(id, raw=True)` to see the exact
    ...
    -- and call `update(id, type="rsk", content, offset=..., limit=...)`
  ...
  - **Whole-body replace** (a multi-section change, or whenever you are
    uncertain about the line range): call `update(id, type="rsk", content)`
    ...
    regardless of what you submit; only `updated` changes. Keep both
    assessments in their mandated shape (`### Probability {1..5}` then
    `### Impact {1..5}` under each H2, values in 1..5) and the strategy
    one of the four TARA words, or the replace fails validation.
- A change to `status` -> `set_status(id, type="rsk", status)` instead
...
- A change to `classification` ->
  `set_classification(id, type="rsk", classification)` instead --
...
`validate(type="rsk", content=content, full=False)` beforehand to dry-run the new body
