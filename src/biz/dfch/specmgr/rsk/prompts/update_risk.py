...
``rsk/tools/`` surface (``get_rsk``, ``update_rsk``, ``set_status_rsk``,
``validate_rsk``). There is no ``specmgr://rsk/{id}`` resource to point at
...
-- a whole-body replace (``update_rsk``) plus a single, dedicated
status-change path (``set_status_rsk``) -- so the tool-mapping section
...
@mcp.prompt(
    ...
    title="Update a risk",
    description=(
        "Guides the LLM through revising an existing risk by id: reading current "
        "state, applying the requested change with the right tool, and validating."
    ),
)
def update_risk(id: str, instructions: str | None = None) -> str:
    ...
    """
    template = Template(read_packaged_text("rsk", "update_instructions", "md"))
