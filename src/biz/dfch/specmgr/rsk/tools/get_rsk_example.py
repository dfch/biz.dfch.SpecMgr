...
"""``@mcp.tool()`` wrapper: get_rsk_example (Task 3.9).

Returns a complete, valid sample risk document as raw markdown -- useful as
a learning example for drafting a new RSK document by hand, or for an LLM to
see the expected shape without re-deriving it from the JSON Schema alone.
Named ``get_rsk_example`` rather than a bare ``get_example``, mirroring
``get_tsk_example``'s own domain-qualified naming rationale -- tool names
...
@mcp.tool(
    name="get_rsk_example",
    title="Get RSK example",
    description=(
        "Return a complete, valid sample risk document as raw markdown -- frontmatter "
        "and body -- exercising every section, for use as a learning example."
    ),
)
def get_rsk_example() -> str:
    ...
    str
        ...
        frontmatter block.
    """
    return read_packaged_text("rsk", "example")
