...
The generic ``update`` tool in ``general.tools`` (``type="rsk"``) and any
future risk mutation tool (``set_status_rsk``) wrap their whole sequence in
``with rsk_lock(id):``.
...
#: One lock per risk id, created lazily on first use and never
...
@contextmanager
def rsk_lock(id_: str) -> Iterator[None]:
