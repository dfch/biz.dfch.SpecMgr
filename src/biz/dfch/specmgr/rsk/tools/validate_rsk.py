...
from ..models.v1 import Risk, parse_rsk


@mcp.tool(
    name="validate_rsk",
    title="Validate risk content",
    description=(
        "Disk-free, id-free dry run validating risk content. `full=False` (default) "
        "validates body-only content (no frontmatter); `full=True` validates a complete document "
        "(frontmatter + body)."
    ),
)
def validate_rsk(content: str, full: bool = False) -> bool:
    ...
    if full:
        ...
        with wrap_tool_errors(domain="rsk", tool="validate_rsk"):
            parse_rsk(content)
    else:
        if has_frontmatter:
            raise ValueError(
        ...
        with wrap_tool_errors(domain="rsk", tool="validate_rsk", channel=BODY_CHANNEL):
