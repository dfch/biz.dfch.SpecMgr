...
Capture the key design decisions from `doc/adr-tool-plan.md` and `AGENTS.md` as a set of Architecture Decision Records (ADRs) in the specmgr MCP server itself. Each ADR will be self-contained (no forward-references to plan sections), and the source documents will be back-referenced to point to the corresponding ADRs.
...
3. **Frontmatter extension fields (`id`, `version`) + whole-object full-replace update contract**
   - Source: `doc/adr-tool-plan.md` §3, §9a
...
6. **Domain-tree hierarchy code organization**
   - Source: `doc/adr-tool-plan.md` §6; `doc/refactor-domain.md` (entire document)
...
7. **Filesystem is the sole source of truth — no in-memory id→document cache**
   - Source: `doc/adr-tool-plan.md` §7, §9a
...
8. **id/filename/addressing scheme: server-generated UUID, `{id}-{slug}.md`, directory-scan resolution**
   - Source: `doc/adr-tool-plan.md` §9a
...
9. **Expose listing/by-id reads as MCP resources in addition to tools**
   - Source: `doc/adr-tool-plan.md` §8, §9a
...
11. **Narrated prompt surface driving the tool sequence, plus step-gated test variants**
    - Source: `doc/adr-tool-plan.md` §8, §11
...
Edit `doc/adr-tool-plan.md` and `AGENTS.md` in place: insert a short parenthetical at each relevant paragraph/heading linking to the corresponding ADR. Format: `(ADR <id>: "<title>")`.

### Delete `doc/refactor-domain.md`
Once ADR #6 is written, delete `doc/refactor-domain.md`. Its content is fully captured by the ADR; no back-reference is needed since the file is removed entirely. Before deletion, confirm no other file references `doc/refactor-domain.md`.

## Phase C: Verify

- Run `specmgr_validate_adr` on each of the 12 new ADR ids to ensure they parse and validate cleanly.
- Sanity-check `AGENTS.md` and `doc/adr-tool-plan.md` read cleanly after back-reference insertions.
- Confirm `doc/refactor-domain.md` deletion leaves no dangling references.
