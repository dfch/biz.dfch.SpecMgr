# `biz.dfch.specmgr.models.md.markdown_section5_with_comment`

Opt-in `MarkdownSection5` variant allowing a leading explanatory comment.
...
- `from_text(text: 'str', *, _path: 'str' = '', _offset: 'int' = 0) -> 'MarkdownSection5WithComment'`
  Enforce the >=1-other-field constraint, then defer to `MarkdownSection5.from_text`.

- `get_extent(text: 'str') -> 'int'`
  Enforce the >=1-other-field constraint, then defer to `MarkdownSection5.get_extent`.
...
- `validate_headings(self) -> 'MarkdownSection5'`
  Validate heading level and no nested headings.

  Base class validates the heading triple structure.
  This validates: specific tag h5 and no nested headings in tokens [3:].
