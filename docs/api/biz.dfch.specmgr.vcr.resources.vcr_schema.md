# `biz.dfch.specmgr.vcr.resources.vcr_schema`

Resource: specmgr://vcr/schema (Task 3.1, packaged data).

Reads VCR's generated JSON Schema from the packaged data copy
(``vcr/data/vcr_schema.json``, via ``general.tools._packaged_data.read_packaged_text``)
rather than ``docs/vcr_schema.json`` directly -- the latter is only readable
...
``docs/vcr_schema.json`` by a dedicated pre-commit hook/CI step that runs
``specmgr schema --type vcr --output-dir src/biz/dfch/specmgr/vcr/data``
-- the same generator as ``docs/vcr_schema.json``, just a second
``--output-dir``, so no bespoke copy logic exists in ``commands/schema.py``.
Deliberately does not import ``commands.schema.generate_vcr_schema()``
...
file it reads is a ``vcr/models/v1``-derived artifact -- see
`.specmgr/feat/feat-6-requirement-artifact/README.md`'s Decisions Made for
the original precedent.

## Functions

### `vcr_schema() -> 'dict[str, Any]'`

Return the parsed contents of VCR's packaged JSON Schema.

Reads the packaged copy (``vcr/data/vcr_schema.json``) fresh
...
with ``docs/vcr_schema.json`` by a dedicated pre-commit hook/CI step),
...
------
FileNotFoundError
    If the packaged ``vcr_schema.json`` is missing.
