# `biz.dfch.specmgr.sop.models.v1.document`

Pydantic model for a full Standard Operating Procedure document (frontmatter + body).

Mirrors `dec.models.v1.document.DecDocument`'s own frontmatter+body pairing.
``SopDocument`` holds no file/id/path information itself -- that lives on
``frontmatter.id``, same convention as ``SopFrontmatter.id``.
...
``SopFrontmatter`` and ``.content`` as ``Sop.from_text(...)`` separately,
then constructs a ``SopDocument`` from the two already-parsed pieces -- see
``sop/models/v1/parser.py::parse_sop`` for that glue.
