...
``tools`` (e.g. ``mdformat``, ``update``, ``set_status``) operate on raw
markdown files and are registered as ``@mcp.tool()`` functions. ``resources``
(e.g. ``version``, ``iso25010``, ``rasci``) are registered as
``@mcp.resource()`` functions. ``prompts`` (e.g. ``compact_history``)
return instructional text and are registered as ``@mcp.prompt()``
