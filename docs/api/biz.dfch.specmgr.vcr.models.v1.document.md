# `biz.dfch.specmgr.vcr.models.v1.document`

Pydantic model for a full verification case record document (frontmatter + body).

Mirrors `dec.models.v1.document.DecDocument`'s own frontmatter+body pairing.
``VcrDocument`` holds no file/id/path information itself -- that lives on
``frontmatter.id``, same convention as ``VcrFrontmatter.id``.
...
``VcrFrontmatter`` and ``.content`` as ``Vcr.from_text(...)`` separately,
then constructs a ``VcrDocument`` from the two already-parsed pieces -- see
``vcr/models/v1/parser.py::parse_vcr`` for that glue.

## Classes

### `VcrDocument`

A full verification case record document: YAML frontmatter and body.

Attributes
----------
frontmatter:
    The YAML frontmatter block. See :class:`VcrFrontmatter`.
body:
    The parsed verification case record sections. See :class:`Vcr`.
