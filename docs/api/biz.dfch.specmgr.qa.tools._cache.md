# `biz.dfch.specmgr.qa.tools._cache`
...
### `_parse(path: 'Path') -> 'QaDocument'`

Read and parse ``path``'s full text into a :class:`QaDocument` (the cache's own ``parse_fn``).


### `invalidate_qa_cache(path: 'Path') -> 'None'`
...
### `reconcile_qa_cache(live_paths: 'Iterable[Path]') -> 'None'`
