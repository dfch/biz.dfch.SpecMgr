# `biz.dfch.specmgr.vcr.tools._cache`
...
### `_parse(text: 'str') -> 'VcrDocument'`

Parse ``text`` into a :class:`VcrDocument` (the cache's own ``parse_fn`` -- Phase 6: text in, not ``Path``).
...
### `invalidate_vcr_cache(path: 'Path') -> 'None'`
...
### `reconcile_vcr_cache(live_paths: 'Iterable[Path]') -> 'None'`
