...
- **Cache:** a single global, process-local, `blake2b` content-hash-validated in-memory vector cache (`general/tools/_embedding_cache.py`), keyed `(domain, resolved path) -> (hash, vector)`, mirroring ADR bfd76370's validation guarantee and its lock-ordering rule (cache lock innermost, never held across embedding computation). Vectors are stored in the provider's native arrays. No on-disk persistence. Lifecycle wiring: the generic `delete` tool invalidates the removed path's entry; `set_feat_id` moves it.
