...
class _FixedDatetime(datetime):
    """``datetime`` stand-in with a frozen ``now`` (the ``offset = 1`` ≡ whole-body equivalence test)."""
        ...
        for case in _CASES:
            with self.subTest(doc_type=case.doc_type):
                ...
                with self.assertRaises(ValueError):
                    update(id="no-such-id-" + case.doc_type, type=case.doc_type, content="frag", limit=2)
                with self.assertRaises(ValueError):
    ...
    def test_range_mode_raises_domain_not_found_for_unknown_id(self) -> None:
        ...
        for case in _CASES:
            with self.subTest(doc_type=case.doc_type):
                ...
                with self.assertRaises(case.not_found_error):
                    update(id="no-such-id", type=case.doc_type, content="frag", offset=1, limit=1)
