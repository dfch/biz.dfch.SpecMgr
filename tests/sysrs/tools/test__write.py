...
"""Tests for ``sysrs.tools._write.write_sysrs_file`` (shared create/update write helper)."""
...
from biz.dfch.specmgr.sysrs.models.v1 import SysrsFrontmatter, parse_sysrs
from biz.dfch.specmgr.sysrs.tools._write import write_sysrs_file
...
class TestWriteSysrsFile(unittest.TestCase):
    """Tests for write_sysrs_file."""
    ...
    def test_writes_frontmatter_and_body_that_round_trips(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_sysrs_file(path, frontmatter, _BODY)

            self.assertTrue(path.exists())
            document = parse_sysrs(path.read_text(encoding="utf-8"))
    ...
    def test_file_ends_with_exactly_one_trailing_newline(self) -> None:
        ...
        frontmatter = SysrsFrontmatter(id="some-id")
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "doc.md"
            write_sysrs_file(path, frontmatter, _BODY)
