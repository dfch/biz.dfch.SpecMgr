...
"""Tests for the `specmgr://sysrs/schema` resource (`sysrs.resources.sysrs_schema.sysrs_schema`)."""
...
from biz.dfch.specmgr.commands.schema import generate_sysrs_schema
from biz.dfch.specmgr.general.tools import _packaged_data
from biz.dfch.specmgr.sysrs.models.v1 import SCHEMA_COMMENT_VERSION
from biz.dfch.specmgr.sysrs.resources.sysrs_schema import sysrs_schema

_DIALECT = "https://json-schema.org/draft/2020-12/schema"


class TestSysrsSchemaResource(unittest.TestCase):
    """Tests for the `sysrs_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged sysrs_schema.json, without any patching."""
        sut = sysrs_schema
    ...
    def test_matches_fresh_generate_sysrs_schema_output(self):
        """ACC-007: the resource must equal a fresh `generate_sysrs_schema()` output (parsed JSON)."""
        sut = sysrs_schema

        result = sut()

        self.assertEqual(result, json.loads(generate_sysrs_schema()))
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "sysrs_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged sysrs_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
