...
"""Tests for the `specmgr://tsk/schema` resource (`tsk.resources.tsk_schema.tsk_schema`)."""
...
class TestTskSchemaResource(unittest.TestCase):
    """Tests for the `tsk_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged tsk_schema.json, without any patching."""
        sut = tsk_schema
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "tsk_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged tsk_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
