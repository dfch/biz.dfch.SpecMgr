...
"""Tests for the `specmgr://prb/schema` resource (`prb.resources.prb_schema.prb_schema`)."""
...
class TestPrbSchemaResource(unittest.TestCase):
    """Tests for the `prb_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged prb_schema.json, without any patching."""
        sut = prb_schema
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "prb_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged prb_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
