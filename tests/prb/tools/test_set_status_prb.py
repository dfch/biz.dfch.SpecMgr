...
"""Tests for the ``set_status_prb`` ``@mcp.tool()`` wrapper (Task 3.5)."""
...
class TestSetStatusPrb(TempPrbDirTestCase):
    """Tests for the set_status_prb tool."""

    def test_sets_status_and_bumps_updated(self) -> None:
        """set_status_prb must write the new status and a fresh `updated` timestamp."""
        original = self.existing_prb()

        result = set_status_prb(original.frontmatter.id, "active")
    ...
    def test_body_is_left_unchanged(self) -> None:
        """set_status_prb must not alter the body at all."""
        original = self.existing_prb()

        set_status_prb(original.frontmatter.id, "active")

        on_disk = parse_prb(self._find_path(original.frontmatter.id).read_text(encoding="utf-8"))
    ...
    def test_written_file_round_trips_via_parse_prb(self) -> None:
        ...
        set_status_prb(original.frontmatter.id, "resolved")

        on_disk = parse_prb(self._find_path(original.frontmatter.id).read_text(encoding="utf-8"))
        self.assertEqual(on_disk.frontmatter.status, "resolved")

    def test_raises_not_found_for_unknown_id(self) -> None:
        """set_status_prb must raise PrbNotFoundError for an id with no matching file."""
        with self.assertRaises(PrbNotFoundError):
            set_status_prb("no-such-id", "active")
    ...
    def test_invalid_status_raises_and_leaves_file_unchanged(self) -> None:
        ...
        with self.assertRaises(ValidationError):
            set_status_prb(original.frontmatter.id, "not-a-real-status")
