...
"""Tests for the ``get_rsk_template`` ``@mcp.tool()`` wrapper (Task 3.9)."""
...
class TestGetRskTemplateTool(unittest.TestCase):
    ...
    def test_packaged_template_round_trips_through_parse_rsk(self) -> None:
        """The committed template must be a fully-parseable risk document (all mandatory
        sections present, both 5x5 assessments, a valid TARA word).
        """
        result = get_rsk_template()

        document = parse_rsk(result)

        self.assertEqual(document.frontmatter.status, "open")
        self.assertEqual(document.body.text, "Level 1 Heading is the Title of the Risk")
        self.assertEqual(document.body.initial_assessment.probability.value, 3)
        ...
        self.assertEqual(document.body.scope.items[0].text, "An affected system or component")
