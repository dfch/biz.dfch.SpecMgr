...
"""Tests for the ``update_vcr`` ``@mcp.prompt()`` (Task 3.2).

``update_vcr`` (the prompt) only ever returns instructional text -- it
never calls ``get_vcr``/``question``/``update``/``set_status`` (the tools)
...
from biz.dfch.specmgr.vcr.prompts.update_vcr import update_vcr


class TestUpdateVcrPrompt(unittest.TestCase):
    """Tests for the update_vcr prompt."""
    ...
    def test_instructions_interpolated_when_given(self):
        ...
        result = update_vcr("id-abc-123", instructions="Mark AC-004 as verified.")
        self.assertIn("Mark AC-004 as verified.", result)
    ...
    def test_instructions_match_packaged_file(self):
        ...
        vcr/data/vcr_update_instructions.md -- evidence the text comes from packaged data."""
        result = update_vcr("id-abc-123")
        self.assertIn("Never assume prior state", result)

    def test_mentions_get_vcr_tool_first(self):
        """The prompt must instruct the LLM to call get_vcr first,
        before the generic `update` write call."""
        result = update_vcr("id-abc-123")
        self.assertIn("get_vcr(id)", result)
        self.assertLess(result.index("get_vcr(id)"), result.index('update(id, type="vcr", content)'))

    def test_mentions_both_generic_mutation_tools(self):
        """Both the generic `update` (type="vcr") and `set_status`
        (type="vcr") call shapes must be named."""
        result = update_vcr("id-abc-123")
        for tool in ('update(id, type="vcr", content)', 'set_status(id, type="vcr", status)'):
    ...
    def test_mentions_showing_which_sections_are_present(self):
        ...
        result = " ".join(update_vcr("id-abc-123").split())
    ...
    def test_mentions_set_status_as_separate_optional_followup(self):
        """The generic `set_status` tool (type="vcr") must be framed as a separate,
        optional follow-up, with the VCR-specific status vocabulary."""
        result = update_vcr("id-abc-123")
    ...
    def test_mentions_dtais_resource(self):
        """The prompt must point at specmgr://dtais for method-word guidance."""
        result = update_vcr("id-abc-123")
        self.assertIn("specmgr://dtais", result)

    def test_instructions_loaded_from_packaged_data_file(self):
        """The instructional text must come from vcr/data/vcr_update_instructions.md,
        not an inline Python string -- reads fresh on every call, no cache."""
        with tempfile.TemporaryDirectory() as tmp:
            instructions_path = Path(tmp) / "vcr_update_instructions.md"
            ...
            with mock.patch.object(_packaged_data, "packaged_data_path", return_value=instructions_path):
                first = update_vcr("id-abc-123", instructions="Mark AC-004 as verified.")
                instructions_path.write_text("second $id / $instructions", encoding="utf-8")
                second = update_vcr("id-abc-123", instructions="Mark AC-004 as verified.")

            self.assertEqual(first, "first id-abc-123 / Mark AC-004 as verified.")
            self.assertEqual(second, "second id-abc-123 / Mark AC-004 as verified.")
