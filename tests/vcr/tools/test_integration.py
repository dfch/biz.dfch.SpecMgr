...
-> ``list_vcr`` (status reflected) -> ``validate_vcr`` -> ``delete_vcr``
(stub) -- against a real temporary docs directory.
...
class TestVcrLifecycleIntegration(TempVcrDirTestCase):
    ...
    def test_list_create_get_list_update_set_status_get_list_validate_delete_roundtrip(self) -> None:
        ...
        list_vcr -> validate_vcr -> delete_vcr, live."""
        ...
        # 9. delete_vcr: stub must always raise NotImplementedError, unconditionally.
        with self.assertRaises(NotImplementedError):
            delete_vcr(vcr_id)
        # The document must still exist afterward -- the stub must not touch the filesystem.
        self.assertEqual(get_vcr(vcr_id).frontmatter.id, vcr_id)
