...
"""Tests for ``vcr.tools._write.write_vcr_file`` (shared create/update write helper)."""
...
from biz.dfch.specmgr.vcr.tools._write import write_vcr_file
...
class TestWriteVcrFile(unittest.TestCase):
    """Tests for write_vcr_file."""
    ...
    def test_writes_frontmatter_and_body_that_round_trips(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_vcr_file(path, frontmatter, _BODY)

            self.assertTrue(path.exists())
            document = parse_vcr(path.read_text(encoding="utf-8"))
    ...
    def test_file_ends_with_exactly_one_trailing_newline(self) -> None:
        ...
        frontmatter = VcrFrontmatter(id="some-id")
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "doc.md"
            write_vcr_file(path, frontmatter, _BODY)
