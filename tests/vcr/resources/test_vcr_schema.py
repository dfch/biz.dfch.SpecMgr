...
"""Tests for the `specmgr://vcr/schema` resource (`vcr.resources.vcr_schema.vcr_schema`)."""
...
from biz.dfch.specmgr.commands.schema import generate_vcr_schema
from biz.dfch.specmgr.general.tools import _packaged_data
from biz.dfch.specmgr.vcr.models.v1 import SCHEMA_COMMENT_VERSION
from biz.dfch.specmgr.vcr.resources.vcr_schema import vcr_schema

_DIALECT = "https://json-schema.org/draft/2020-12/schema"


class TestVcrSchemaResource(unittest.TestCase):
    """Tests for the `vcr_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged vcr_schema.json, without any patching."""
        sut = vcr_schema
    ...
    def test_matches_fresh_generate_vcr_schema_output(self):
        """ACC-006: the resource must equal a fresh `generate_vcr_schema()` output (parsed JSON)."""
        sut = vcr_schema

        result = sut()

        self.assertEqual(result, json.loads(generate_vcr_schema()))
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "vcr_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged vcr_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
