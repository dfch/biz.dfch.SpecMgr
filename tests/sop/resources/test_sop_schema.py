...
"""Tests for the `specmgr://sop/schema` resource (`sop.resources.sop_schema.sop_schema`)."""
...
from biz.dfch.specmgr.commands.schema import generate_sop_schema
from biz.dfch.specmgr.general.tools import _packaged_data
from biz.dfch.specmgr.sop.models.v1 import SCHEMA_COMMENT_VERSION
from biz.dfch.specmgr.sop.resources.sop_schema import sop_schema

_DIALECT = "https://json-schema.org/draft/2020-12/schema"


class TestSopSchemaResource(unittest.TestCase):
    """Tests for the `sop_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged sop_schema.json, without any patching."""
        sut = sop_schema
    ...
    def test_matches_fresh_generate_sop_schema_output(self):
        """ACC-004: the resource must equal a fresh `generate_sop_schema()` output (parsed JSON)."""
        sut = sop_schema

        result = sut()

        self.assertEqual(result, json.loads(generate_sop_schema()))
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "sop_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged sop_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
