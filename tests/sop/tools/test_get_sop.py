...
class TestGetSop(unittest.TestCase):
    ...
    def test_raw_returns_body_text_via_shared_helper(self) -> None:
        ...
        result = get_sop(created.id, raw=True)
    ...
    def test_raw_line_coordinates_index_into_the_splice_target(self) -> None:
        ...
        created = create_sop(_MINIMAL_BODY)
        lines = get_sop(created.id, raw=True).splitlines()
        k = lines.index("Provision accounts for new hires.") + 1
        replacement = "Provision accounts for all new hires."

        update(id=created.id, type="sop", content=replacement, offset=k, limit=1)

        new_lines = get_sop(created.id, raw=True).splitlines()
    ...
    def test_raw_windowed_read_returns_the_requested_slice(self) -> None:
        ...
        created = create_sop(_MINIMAL_BODY)
        doc_id = created.id
        lines = get_sop(doc_id, raw=True).splitlines()

        result = get_sop(doc_id, raw=True, offset=2, limit=3)
    ...
    def test_raw_windowed_read_clamps_out_of_range_coordinates(self) -> None:
        ...
        self.assertEqual(get_sop(doc_id, raw=True, offset=len(lines) + 1), "")
        self.assertEqual(get_sop(doc_id, raw=True, offset=len(lines) + 10, limit=5), "")
        self.assertEqual(get_sop(doc_id, raw=True, offset=2, limit=len(lines) + 10), "\n".join(lines[1:]) + "\n")
    ...
    def test_coordinates_with_raw_false_raise_value_error(self) -> None:
        ...
        with self.assertRaises(ValueError) as ctx:
            get_sop(created.id, raw=False, offset=2, limit=3)
        message = str(ctx.exception)
        self.assertIn("raw", message)
        self.assertIn("offset", message)
        with self.assertRaises(ValueError):
            get_sop(created.id, raw=False, limit=3)
    ...
    def test_windowed_raw_read_coordinates_index_into_the_splice_target(self) -> None:
        ...
        lines = get_sop(doc_id, raw=True).splitlines()
        k, m = 3, 3
        window = get_sop(doc_id, raw=True, offset=k, limit=m)
        self.assertEqual(window, "\n".join(lines[k - 1 : k - 1 + m]) + "\n")
        replacement = "## Purpose\n\nProvision accounts for all new hires."

        update(id=doc_id, type="sop", content=replacement, offset=k, limit=m)

        new_lines = get_sop(doc_id, raw=True).splitlines()
    ...
    def test_raw_false_returns_parsed_document_as_before(self) -> None:
        ...
        result = get_sop(created.id, raw=False)
        default = get_sop(created.id)

        self.assertIsInstance(result, SopDocument)
        self.assertEqual(result, default)

    def test_raw_unknown_id_raises_not_found_in_both_modes(self) -> None:
        """raw=True and raw=False must both raise SopNotFoundError for an unknown id, windowed raw
        reads included."""
        create_sop(_MINIMAL_BODY)
        ...
        with self.assertRaises(SopNotFoundError):
            get_sop(_MISSING_UUID, raw=True, offset=2, limit=3)
        with self.assertRaises(SopNotFoundError):
