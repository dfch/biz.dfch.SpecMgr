...
"""Tests for ``sop.tools._lock.sop_lock`` (per-document mutation serialization)."""
...
from biz.dfch.specmgr.sop.tools._lock import sop_lock


def _run_and_track_overlap(ids: list[str], hold_seconds: float = 0.05) -> int:
    """Run one thread per entry in ``ids``, each holding ``sop_lock(id_)`` for
...
class TestSopLock(unittest.TestCase):
    """Tests for sop_lock's serialization behaviour."""

    def test_serializes_calls_against_the_same_id(self) -> None:
        """Concurrent sop_lock(id) calls for the same id must never overlap."""
        max_active = _run_and_track_overlap(["same-id"] * 5)
        self.assertEqual(max_active, 1)

    def test_does_not_serialize_calls_against_different_ids(self) -> None:
        """sop_lock calls for distinct ids must be able to run concurrently."""
