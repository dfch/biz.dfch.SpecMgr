...
"""Tests for the ``list_gol`` ``@mcp.tool()`` wrapper (Task 3.9, ACC-004).

Paged from day one (ADR ec9f5262-9912-49d0-903f-fcfb54f28c13) -- there is no
former ``specmgr://gol/list`` resource these tests could have migrated from,
...
from biz.dfch.specmgr.gol.models.v1 import GolSummary
from biz.dfch.specmgr.gol.tools._paths import ensure_gol_base_dir
...
_OTHER_BODY = _MINIMAL_BODY.replace("Competitive Engines in Consumer Vehicles", "Affordable Powertrains")


def _body_with_title(title: str) -> str:
    return _MINIMAL_BODY.replace("Competitive Engines in Consumer Vehicles", title)


class TestListGol(unittest.TestCase):
    """Tests for the list_gol tool."""
    ...
    def test_returns_summaries_and_reports_malformed_file_as_a_failed_entry(self) -> None:
        first = create_gol(_MINIMAL_BODY)
        second = create_gol(_OTHER_BODY)

        base_dir = ensure_gol_base_dir()
        broken_path = base_dir / "broken.md"
        broken_path.write_text("not a valid goal, no headings at all", encoding="utf-8")
        ...
        self.assertEqual(
            titles, {"Competitive Engines in Consumer Vehicles", "Affordable Powertrains", "<failed to parse>"}
        )
    ...
    def test_empty_result_for_missing_directory(self) -> None:
        self.assertFalse((self.docs_root / "gol").exists())
    ...
    def test_default_page_size_with_more_than_25_documents(self) -> None:
        for i in range(26):
            create_gol(_body_with_title(f"Goal Number {i:02d}"))
    ...
    def test_offset_selects_the_next_page(self) -> None:
        ...
        first_page = list_gol(max_results=2)
        second_page = list_gol(max_results=2, offset=2)
    ...
    def test_truncated_false_when_offset_exactly_at_the_end(self) -> None:
        ...
        last_page = list_gol(max_results=25, offset=25)
        past_end = list_gol(max_results=25, offset=26)
    ...
    def test_total_and_error_count_reflect_the_full_directory_regardless_of_paging(self) -> None:
        for i in range(5):
            create_gol(_body_with_title(f"Goal Number {i:02d}"))
        base_dir = ensure_gol_base_dir()
        (base_dir / "broken.md").write_text("not a valid goal, no headings at all", encoding="utf-8")

        sut = list_gol(max_results=1, offset=1)
