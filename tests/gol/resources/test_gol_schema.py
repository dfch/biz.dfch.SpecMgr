...
"""Tests for the `specmgr://gol/schema` resource (`gol.resources.gol_schema.gol_schema`)."""
...
from biz.dfch.specmgr.gol.resources.gol_schema import gol_schema


class TestGolSchemaResource(unittest.TestCase):
    """Tests for the `gol_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged gol_schema.json, without any patching."""
        sut = gol_schema
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "gol_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged gol_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
