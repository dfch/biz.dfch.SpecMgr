...
"""Tests for the ``qa_list`` resource (``specmgr://qa/list``)."""
...
from biz.dfch.specmgr.qa.resources.qa_list import qa_list
...
class TestQaListResource(unittest.TestCase):
    """Tests for the `qa_list` resource function (`specmgr://qa/list`)."""
    ...
    def test_returns_summaries_and_skips_malformed_file(self) -> None:
        """qa_list must return exactly the valid QA documents, silently skipping a broken file."""
    ...
    def test_empty_list_for_missing_directory(self) -> None:
        """qa_list must return an empty list when the base directory does not exist."""
        self.assertFalse((self.docs_root / "qa").exists())
        self.assertEqual(qa_list(), [])
