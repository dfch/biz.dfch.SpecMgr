...
class TestGetReq(unittest.TestCase):
    ...
    def test_raw_returns_body_text_via_shared_helper(self) -> None:
        ...
        result = get_req(created.id, raw=True)
    ...
    def test_raw_line_coordinates_index_into_the_splice_target(self) -> None:
        ...
        created = create_req(_MINIMAL_BODY)
        lines = get_req(created.id, raw=True).splitlines()
        k = lines.index("If the engine becomes too hot, the lifetime of the system decreases.") + 1
        replacement = "Updated description text."

        update(id=created.id, type="req", content=replacement, offset=k, limit=1)

        new_lines = get_req(created.id, raw=True).splitlines()
    ...
    def test_raw_windowed_read_returns_the_requested_slice(self) -> None:
        ...
        result = get_req(doc_id, raw=True, offset=2, limit=3)
    ...
    def test_raw_windowed_read_clamps_out_of_range_coordinates(self) -> None:
        ...
        self.assertEqual(get_req(doc_id, raw=True, offset=len(lines) + 1), "")
        self.assertEqual(get_req(doc_id, raw=True, offset=len(lines) + 10, limit=5), "")
        self.assertEqual(get_req(doc_id, raw=True, offset=2, limit=len(lines) + 10), "\n".join(lines[1:]) + "\n")
    ...
    def test_coordinates_with_raw_false_raise_value_error(self) -> None:
        ...
        with self.assertRaises(ValueError) as ctx:
            get_req(created.id, raw=False, offset=2, limit=3)
        message = str(ctx.exception)
    ...
    def test_windowed_raw_read_coordinates_index_into_the_splice_target(self) -> None:
        ...
        lines = get_req(doc_id, raw=True).splitlines()
        k, m = 5, 3
        window = get_req(doc_id, raw=True, offset=k, limit=m)
        self.assertEqual(window, "\n".join(lines[k - 1 : k - 1 + m]) + "\n")
        replacement = "## Description\n\nUpdated description text."

        update(id=doc_id, type="req", content=replacement, offset=k, limit=m)

        new_lines = get_req(doc_id, raw=True).splitlines()
    ...
    def test_raw_false_returns_parsed_document_as_before(self) -> None:
        ...
        result = get_req(created.id, raw=False)
        default = get_req(created.id)

        self.assertIsInstance(result, ReqDocument)
        self.assertEqual(result, default)

    def test_raw_unknown_id_raises_not_found_in_both_modes(self) -> None:
        """raw=True and raw=False must both raise ReqNotFoundError for an unknown id, windowed raw
        reads included."""
        create_req(_MINIMAL_BODY)
        ...
        with self.assertRaises(ReqNotFoundError):
            get_req(_MISSING_UUID, raw=True, offset=2, limit=3)
        with self.assertRaises(ReqNotFoundError):
