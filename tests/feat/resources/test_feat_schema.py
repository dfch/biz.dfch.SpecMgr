...
"""Tests for the `specmgr://feat/schema` resource (`feat.resources.feat_schema.feat_schema`)."""
...
from biz.dfch.specmgr.commands.schema import generate_feat_schema
from biz.dfch.specmgr.feat.models.v1 import SCHEMA_COMMENT_VERSION
from biz.dfch.specmgr.feat.resources.feat_schema import feat_schema
from biz.dfch.specmgr.general.tools import _packaged_data

_DIALECT = "https://json-schema.org/draft/2020-12/schema"


class TestFeatSchemaResource(unittest.TestCase):
    """Tests for the `feat_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged feat_schema.json, without any patching."""
        sut = feat_schema
    ...
    def test_matches_fresh_generate_feat_schema_output(self):
        """ACC-007: the resource must equal a fresh `generate_feat_schema()` output (parsed JSON)."""
        sut = feat_schema

        result = sut()

        self.assertEqual(result, json.loads(generate_feat_schema()))
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "feat_schema.json"
    ...
    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged feat_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
