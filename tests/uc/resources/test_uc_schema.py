...
"""Tests for the `specmgr://uc/schema` resource (`uc.resources.uc_schema.uc_schema`)."""
...
from biz.dfch.specmgr.uc.resources.uc_schema import uc_schema


class TestUcSchemaResource(unittest.TestCase):
    """Tests for the `uc_schema` resource function."""

    def test_returns_parsed_repo_schema_by_default(self):
        """Against the real, committed packaged uc_schema.json, without any patching."""
        sut = uc_schema
    ...
    def test_returns_parsed_dict_for_a_given_file(self):
        ...
        payload = {"$schema": "https://json-schema.org/draft/2020-12/schema", "$comment": "v2", "properties": {}}
        with tempfile.TemporaryDirectory() as tmp:
            schema_path = Path(tmp) / "uc_schema.json"
    ...
    def test_reads_fresh_on_every_call(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(_packaged_data, "packaged_data_path", return_value=schema_path):
                ...
                first = sut()
                schema_path.write_text(json.dumps({"$comment": "v3"}), encoding="utf-8")
                second = sut()

            self.assertEqual(first["$comment"], "v2")
            self.assertEqual(second["$comment"], "v3")

    def test_raises_file_not_found_when_schema_missing(self):
        """A missing packaged uc_schema.json must propagate FileNotFoundError uncaught."""
        with tempfile.TemporaryDirectory() as tmp:
