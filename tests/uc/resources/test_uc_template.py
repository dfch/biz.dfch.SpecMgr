...
"""Tests for the `specmgr://uc/template` resource (`uc.resources.uc_template.uc_template`)."""
...
from biz.dfch.specmgr.uc.resources.uc_template import uc_template
from biz.dfch.specmgr.uc.tools.get_uc_template import get_uc_template


class TestUcTemplateResource(unittest.TestCase):
    """Tests for the uc_template resource function."""
    ...
    def test_matches_the_get_uc_template_tool(self):
        """The resource and the tool must return identical content -- same underlying reader."""
        self.assertEqual(uc_template(), get_uc_template())
